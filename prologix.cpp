...
#include <mmsystem.h>
#include <windowsx.h>
#include <ws2tcpip.h> // For modern Winsock APIs
#include <setupapi.h>
...
// Local helper function, needs to use inet_ntop
                   ...
                   S32          show_port)
{
   ...
   if (inet_ntop(AF_INET, &(address->sin_addr), ip_buffer, sizeof(ip_buffer)) == NULL) {
       strcpy(ip_buffer, "UnknownIP"); // Fallback
   }
...
{                    
   ...
   for (S32 pp=0; (pp < (msecs/10)) && (n_enum_ports < MAX_PORTS); pp++)
      ...
      sockaddr_in TCP_address; // This is used to construct the name string, not for actual connection
      char ip_buffer[INET_ADDRSTRLEN];
      if (inet_ntop(AF_INET, (struct in_addr*)entry.ip, ip_buffer, sizeof(ip_buffer)) == NULL) {
          strcpy(ip_buffer, "UnknownIP");
      }
...
void enumerate_ports(void)
{
   ...
   //
   // Enumerate VISA resources
   //
   HMODULE hVISA = LoadLibrary("visa32.dll");
   if (hVISA != NULL)
      {
      ...
      typedef ViStatus (__stdcall * P_viFindRsrc)(ViSession sesn, ViString expr, ViUInt32 * vi, ViPUInt32 retCnt, ViChar desc[]);
      typedef ViStatus (__stdcall * P_viFindNext)(ViUInt32 vi, ViChar desc[]);
      typedef ViStatus (__stdcall * P_viClose)(ViUInt32 vi);

      P_viOpenDefaultRM p_viOpenDefaultRM = (P_viOpenDefaultRM) GetProcAddress(hVISA, "viOpenDefaultRM");
      P_viFindRsrc p_viFindRsrc = (P_viFindRsrc) GetProcAddress(hVISA, "viFindRsrc");
      P_viFindNext p_viFindNext = (P_viFindNext) GetProcAddress(hVISA, "viFindNext");
      P_viClose p_viClose = (P_viClose) GetProcAddress(hVISA, "viClose");
      ...
      if (p_viOpenDefaultRM && p_viFindRsrc && p_viFindNext && p_viClose && 
          p_viOpenDefaultRM(&defaultRM) == 0)
         {
         if (p_viFindRsrc(defaultRM, "?*::INSTR", &findList, &retCnt, desc) == 0)
            {
            do
               {
               if (n_enum_ports < MAX_PORTS)
                  {
                  _snprintf(enum_ports[n_enum_ports].name, sizeof(enum_ports[n_enum_ports].name), "VISA Resource (%s)", desc);
                  _snprintf(enum_ports[n_enum_ports].interface_settings, sizeof(enum_ports[n_enum_ports].interface_settings), "%s", desc);
                  enum_ports[n_enum_ports].type = EP_VISA;
                  SendDlgItemMessage(hwnd, R_PORTLIST, LB_ADDSTRING, 0, (LPARAM) enum_ports[n_enum_ports++].name);
                  }
