...
S32            show_iband_band       =  0;
S32            show_ichans_band      =  0;
S32            show_GPS_band         =  0;
S32            show_fps              =  1;
...
#define IDM_IBAND        139
#define IDM_ICHANS       140
...
// Acquire options begin at IDM_PAUSE_RESUME+1
...
//
// Iridium simplex (band limits)
//

CHANNEL CHANS_Iband[1] = {{ 1626.25, 0.25, "Simplex band",  200, 0, 0   }};

BAND Iband = { CHANS_Iband,
               1,
               0 };

//
// Iridium simplex (all channels)
//
//

CHANNEL CHANS_Ichan[12] = {{ 1626.020833, 0.016, "1",  0, 90,  0 },
                           { 1626.062500, 0.016, "2",  0, 90,  0 },
                           { 1626.104167, 0.016, "3",  0, 220, 0 },
                           { 1626.145833, 0.016, "4",  0, 220, 0 },
                           { 1626.187500, 0.016, "5",  0, 90,  0 },
                           { 1626.229167, 0.016, "6",  0, 90,  0 },
                           { 1626.270833, 0.016, "7",  0, 220, 0 },
                           { 1626.312500, 0.016, "8",  0, 90,  0 },
                           { 1626.354167, 0.016, "9",  0, 90,  0 },
                           { 1626.395833, 0.016, "10", 0, 220, 0 },
                           { 1626.437500, 0.016, "11", 0, 220, 0 },
                           { 1626.479167, 0.016, "12", 0, 90,  0 }};

BAND Ichans = { CHANS_Ichan,
                12,
                0 };
...
time_t SOURCE_fetch_data(SINGLE *dest)
{
   ...
   if (!mswp)
      ...
      while (start_Hz < mx_Hz)
         {
         DOUBLE stop_Hz = std::min(mx_Hz, start_Hz + span_size_Hz);

         DOUBLE span_Hz = std::min(span_size_Hz, stop_Hz - start_Hz);
         DOUBLE CF_Hz   = start_Hz + (span_Hz / 2.0);

         SA_query(":SENS:FREQ:CENT %lf HZ;*OPC?\n", CF_Hz - SOURCE_state->F_offset); 
         ...
         switch (resamp_op)
            ...
            default:
               ...
               for (S32 i=0; i < n_src_points; i++, src_Hz += ds_Hz)
                  ...
                  S32 next_d = std::min(n_data_points-1, (S32) (((src_Hz - mn_Hz) / dd_Hz) + 0.5));
      ...
      for (S32 i=0; i < n_data_points; i++)
         {
         dest[i] = (SINGLE) (10.0 * log10(dest64[i] / std::max(destcnt[i],1.0)));
         }
      }
      }
...
void graph_setup(bool force_recreate=FALSE)
{
   ...
   if (force_recreate)
      ...
      rise->set_graph_attributes(std::max(1, WATERFALL_PANE_WIDTH / n_data_points),
                                 RISE_VALUES,
                                 RISE_COLORS,
                                 N_RISE_COLORS,
                                 RISE_VALUES[0] - 10,
   ...
   xy->set_graph_attributes(style,
                            ...
                            XY_BKGND_COLOR,
                            XY_TRACE_COLOR,
                            show_xy_scale ? XY_SCALE_COLOR : XY_BKGND_COLOR,
                            show_xy_scale ? XY_MAJOR_COLOR : XY_BKGND_COLOR,
                            show_xy_scale ? XY_MINOR_COLOR : XY_BKGND_COLOR,
                            XY_CURSOR_COLOR,
                            XY_RUN_COLOR);
   ...
   switch (show_iband_band)
      {
      case 1: CheckMenuItem(hmenu, IDM_IBAND, MF_CHECKED); break;
      case 0: CheckMenuItem(hmenu, IDM_IBAND, MF_UNCHECKED); break;
      }

   switch (show_ichans_band)
      {
      case 1: CheckMenuItem(hmenu, IDM_ICHANS, MF_CHECKED); break;
      case 0: CheckMenuItem(hmenu, IDM_ICHANS, MF_UNCHECKED); break;
      }
...
void main_menu_init(void)
{
   ...
   AppendMenu(pop, MF_STRING, IDM_RESETSW,    "Reset running values\tR");
   AppendMenu(pop, MF_SEPARATOR, 0, NULL);
   ...
   AppendMenu(pop, MF_STRING, IDM_IBAND,      "Iridium simplex band limits\ti");
   AppendMenu(pop, MF_STRING, IDM_ICHANS,     "Iridium simplex channel markers\tI");
   AppendMenu(pop, MF_SEPARATOR, 0, NULL);
...
//         fclose(hack);
         ...
         S32 n_visible_records = std::min(n_readable_records,
                                     n_visible_rows);
...
select_playback:
            ...
            case IDM_RESETSW:    xy->reset_running_values(); xy->clear_max_hold();         graph_setup(); break;
            ...
            case IDM_ABOUT:
               {
                  char about_text[256];
                  _snprintf(about_text, sizeof(about_text), "Spectrum surveillance monitor V%s\n\nAuthor: John Miles, KE5FX (john@miles.io)", VERSION);
                  SAL_alert_box("About SSM.EXE", about_text);
                  break;
               }
...
refresh:
            ...
            case IDM_IBAND:      show_iband_band = !show_iband_band;       graph_setup(); break;
            case IDM_ICHANS:     show_ichans_band = !show_ichans_band;     graph_setup(); break;
...
stop_start:
            ...
            //
            // Q: Toggle high speed acquisition
            //

            case 'Q':
               {
               SOURCE_state->hi_speed_acq = !SOURCE_state->hi_speed_acq;
               graph_setup();
               break;
               }   
               
            //
            // R: Reset running values
            //                            
            ...
            //
            // i/I: Toggle Iridium band/channel boundaries
            //

            case 'i':
               show_iband_band = !show_iband_band;     
               graph_setup(); 
               break; 

            case 'I':
               show_ichans_band = !show_ichans_band;     
               graph_setup(); 
               break; 
...
void AppMain(LPSTR _lpCmdLine)
{
   ...
   //
   // Check for CLI options (longer ones first!!!)
   //
   ...
   while (1)
      ...
      //
      // -interval_ms: Acquisition rate
      //

      option = strstr(lpCmdLine,"-interval_ms");
      ...
      if (option != NULL)
         ...
         while ((*ptr) && (*ptr != ' '))
            {
            interval_ms = (interval_ms * 10) + (*ptr++ - '0');
            }
      ...
      //
      // -V: Enable average-accum mode in X/Y graph
      //

      option = strstr(lpCmdLine,"-V");
      ...
      //
      // -i/-I: Enable Iridium band/channel markers
      //

      option = strstr(lpCmdLine,"-i");

      if (option != NULL)
         {
         show_iband_band = TRUE;
         memmove(option, &option[2], strlen(&option[2])+1);
         continue;
         }
   ...
   for (;;)
      ...
      if ((operation == ACQUIRE) && (!clicking))
         ...
         if ((operation == ACQUIRE) && (!paused)) // (i.e., if not aborted by hardware layer and not paused with Scroll Lock key)
            ...
            S32 cur_ms = timeGetTime();

            if (cur_ms-next_ms >= 0)
               {
               next_ms = cur_ms+interval_ms;
               write_record = TRUE;
               }
      ...
      if (show_iband_band)   xy->draw_band_markers(&Iband);
      if (show_ichans_band)  xy->draw_band_markers(&Ichans);
      ...
      if (acquisition_time > 0)
         ...
         if (rise->query_graph(mouse_x,
            ...
            sprintf(string,"%s: %3.1f dBm @ %5.3lf MHz",
                  sanitize_VFX(ctime(&rise_acq_time)),
               rise_amplitude,
               x_scale);
