...
static C8 *Hz_string_buf(DOUBLE v, bool log_mode)
{
   static C8 buf[256];
   if (log_mode) return log_Hz_string(v, buf, 256);
   return Hz_string(v, buf, 256);
}
...
#define IDM_N9900         518
#define IDM_R3131         519
#define IDM_R3132         520
#define IDM_R3261         521
#define IDM_R3465         522
#define IDM_R3267         523
#define IDM_TR417X        524
#define IDM_MS8604A       525
#define IDM_MS2650        526
#define IDM_FSIQ          527
#define IDM_FSP           528
#define IDM_FSQ           529
#define IDM_FSU           530
                                 ...
                                 S32    force_0dB)
{
   ...
   fprintf(out,";\n");
   fprintf(out,";Generated by PN.EXE V%s of %s\n", VERSION, __DATE__);
   ...
   for (S32 d=min_decade; d <= max_decade; d++)
      ...
      for (S32 half=0; half < 2; half++)
         ...
         if (atoi(size_text) != (S32) (E4400_n_points * sizeof(S32)))
            { // TODO: First run on E4440A PSA with 82357B+Agilent I/O layer yields 404� instead of 2404; subsequent runs OK, per R. Griffiths 2/2010
...
//****************************************************************************
//*                                                                          *
//* Acquire data from SCPI-compatible N9900-series spectrum analyzer         *
//*                                                                          *
//****************************************************************************
               ...
               SINGLE ref = 10000.0F)
{
   ...
   S32 N9900_n_points = 0;

   while (N9900_n_points == 0)
      {
      C8 *text = GPIB_query(":SENS:SWEEP:POINTS?");
      sscanf(text,"%d",&N9900_n_points);
      }

   if (N9900_n_points > MAX_SCPI_POINTS)
      {
      GPIB_disconnect();
   ...
   S32 record_n_points = 100 * (N9900_n_points / 100);
   S32 decile_n_points = record_n_points / 10;
   ...
   for (S32 d=min_decade; d <= max_decade; d++)
      ...
      for (S32 half=0; half < 2; half++)
         ...
         if (key_hit)
            {
            key_hit = FALSE;
            EXIT_N9900(clip, carrier + mult_delta_Hz);
         ...
         GPIB_printf(":INIT:IMM;*WAI;:TRAC1:DATA?");
         ...
         if (atoi(size_text) != (S32) (N9900_n_points * sizeof(S32)))
            { // TODO: First run on E4440A PSA with 82357B+Agilent I/O layer yields 404� instead of 2404; subsequent runs OK, per R. Griffiths 2/2010
            SAL_alert_box("Notice (please report to john@miles.io)","size_text = [%s], n_preamble_bytes = %d\n\n(Expected %d-point sweep)",
               size_text, n_preamble_bytes, N9900_n_points);
            }

         assert(atoi(size_text) == (S32) (N9900_n_points * sizeof(S32)));
         ...
         memset(curve, 
                0, 
                N9900_n_points * sizeof(SINGLE));
   
         memcpy(curve, 
                GPIB_read_BIN(N9900_n_points * sizeof(SINGLE),
                              TRUE,
                              FALSE), 
                N9900_n_points * sizeof(SINGLE));
...
void render_source_list(BOOL show_selection_cursor)
{
   ...
   if (invalid_trace_cache)
      ...
      for (s=0; s < n_data_sources; s++)
         ...
         if (spur_reduction)
            {
            PN_clip_spurs(S->VD, S->VV, width, spur_dB);
            }
   ...
   for (col=0; col < width; col++)
      ...
      if (int_decade_exp != cur_int_decade_exp)
         ...
         decade_label_Hz = (SINGLE) pow(10.0F,int_decade_exp);
   ...
   for (s=0; s < n_data_sources; s++)
      ...
      PN_smooth_trace(S->VW, S->VV, width, n_smooth, alt_smoothing != 0, S->smoothed);

      if (spot_in_col >= 0 && spot_in_col < width)
      {
         S->spot_dBc_Hz = S->smoothed[spot_in_col];
      }

      for (col=0; col < width; col++)
         {
         DOUBLE acc = S->smoothed[col];
      ...
      if (!RMS_needed)
         ...
         PN_calculate_rms_noise(frequency, S->smoothed, S->VV, width, L_column, U_column, S->carrier_Hz, S->RMS_rads, S->CNR, S->resid_FM, S->jitter_s);
         }
      }
   ...
   if (RMS_needed)
      ...
      sprintf(text,"%s Hz",Hz_string_buf(L_offset_Hz, true));
      ...
      sprintf(text,"%s Hz",Hz_string_buf(U_offset_Hz, true));
   ...
   sprintf(working_captions[T_SPOT_AMP  ], "dBc/Hz at %s Hz", Hz_string_buf(spot_offset_Hz, true));
   ...
   for (s=0; s < n_data_sources; s++)
      ...
      sprintf(L->text[T_CARRIER_FREQ],"   %s   ",Hz_string_buf(S->carrier_Hz, false));
      ...
      if (INI_show_legend_field[T_EXT_IF])
         if (S->ext_IF_Hz >= 0)
            sprintf(L->text[T_EXT_IF],"   %s   ",Hz_string_buf(S->ext_IF_Hz, false));
         else
      ...
      if (INI_show_legend_field[T_EXT_LO])
         if (S->ext_LO_Hz >= 0)
            sprintf(L->text[T_EXT_LO],"   %s   ",Hz_string_buf(S->ext_LO_Hz, false));
         else
...
The factory default GPIB address for many HP/Agilent analyzers is 18.");
                  ...
                  else if (DLG_source_ID == IDM_N9900)
                     {
                     SetDlgItemText(hDlg, IDC_HELPBOX,   "\
Specifies the spectrum analyzer's GPIB address as a decimal value.\n\n\
The factory default GPIB address for many HP/Agilent analyzers is 18.\nIf the instrument is connected via TCP/IP, the GPIB address is not used.");
                     }
                  else if (DLG_source_ID == IDM_TEK490)
                     {
                     SetDlgItemText(hDlg, IDC_HELPBOX,   "\
...
{
   char caption[256];
   _snprintf(caption, sizeof(caption), "Composite Noise Measurement Utility version %s by John Miles, KE5FX (john@miles.io)", VERSION);
   MessageBox(hWnd,
   ...
   switch (message)
      ...
      case WM_COMMAND:
         ...
         switch (wParam)
            ...
            case IDM_N9900           : CMD_acquire(IDM_N9900);                 break; 
