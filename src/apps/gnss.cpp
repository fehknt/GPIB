   ...
   virtual UINT thread_proc(void)
      ...
      for (;;)
         ...
         for (S32 i=0; i < cnt; i++)
            ...
            if (ch == 10)
               {
               if (!verify_nmea_checksum(line_buffer)) continue;

               GpggaData data;
               if (parse_gpgga(line_buffer, data))
                  {
                  if (data.fix_quality == 0) continue;

                  lock();
                  S32 hms = (S32) data.UTC;

                  line_UTC_secs    = hms % 100; hms /= 100;
                  line_UTC_mins    = hms % 100; hms /= 100;
                  line_UTC_hours   = hms % 100;

                  line_latitude    = ddmm2degrees(data.latitude,  data.lat_sign);
                  line_longitude   = ddmm2degrees(data.longitude, data.long_sign);

                  line_altitude_m  = data.altitude;
                  line_geoid_sep_m = data.geoid_sep;
