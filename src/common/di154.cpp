...
void DI154::message_sink(DI154MSGLVL level,   
                          C8         *text)
{
   fprintf(stderr, "%s\n",text);
}

void DI154::message_printf(DI154MSGLVL level,
                            C8         *fmt,
                            ...)
{
   C8 buffer[4096];
...
S32 DI154::buf_validate(const U8 *buf)
{
   S32 idx;
...
S32 DI154::buf_decode(const U8      *buf, 
...
BOOL DI154::cmd_send(const U8 *data, S32 len, BOOL report_echo_errors)
{
   for (S32 i=0; i < len; i++)
      {
...
void DI154::close(void)
{
   serial.disconnect();
}

BOOL DI154::open(C8 *port, S32 Hz, U32 analog_chan_mask, U32 digital_chan_mask)
{
   n_chans = 0;
   ...
   if (strncmp((C8 *) data, rs194_signature, SERIAL_NUM_LEN) == 0)
      ...
      for (S32 idx = 0; idx < (S32)sizeof(enable_key); idx++)
         {
...
S32 DI154::read(DI154_READING *data, S32 num_bits)
{
   if (num_bits == 0)
      {
...
DOUBLE DI154::scale(DOUBLE val, DOUBLE pos_val, DOUBLE neg_val, DOUBLE pos_fs, DOUBLE neg_fs)
{
