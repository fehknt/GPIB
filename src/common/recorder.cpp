                                 ...
                                 S32      max_palette_entries)
{
   ...
   *version = file_version;
   ...
   if (max_palette_entries > 0)
      ...
      S32 n_entries = std::min(actual_palette_entry_cnt, 
                          max_palette_entries);
...
S32 RECORDER::write_record       (S32 timestamp,
...
S32 RECORDER::read_record(SINGLE *dest,
                             S32     read_position,
                             DOUBLE *latitude,
                             DOUBLE *longitude,
                             DOUBLE *altitude_m,
                             ...
                             S32     caption_dest_size)
{
   ...
   S32 read_pos = read_header_size +                                                           
                 (read_position * (sizeof(S32) + record_extra_bytes + (read_input_width * sizeof(SINGLE))));
      ...
      while (*S != TAG_EOD)
         ...
         switch (tag)
            ...
            case TAG_CAPTION:
               ...
               memcpy(caption_dest,    
                      S, 
                      std::max(caption_dest_size-1, len));
               break;
               }
