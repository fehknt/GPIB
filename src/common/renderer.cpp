...
// External dependencies defined in apps (e.g. 7470.cpp, pn.cpp)
extern PANE *stage;
extern S32 pen_colors[];
extern S32 standard_pen_colors[];
extern S32 background;
extern C8 antialias;
extern S32 RES_X;
extern S32 RES_Y;
extern U8 in_hpgl;
extern U8 force_lb_term;
extern int user_rotate;
extern int background;
extern int INI_background;
extern U8 plotter_mode;
extern double GL_XMIN, GL_XMAX, GL_YMIN, GL_YMAX;
extern double gl_ip_xmin, gl_ip_xmax, gl_ip_ymin, gl_ip_ymax;
extern double gl_sc_xmin, gl_sc_xmax, gl_sc_ymin, gl_sc_ymax;
extern U8 gl_clip;
extern double gl_clip_x1, gl_clip_y1, gl_clip_x2, gl_clip_y2;
extern int sc_enabled;
...
U8 gl_alt_font;
U8 gl_ct_mode;
double gl_chord_angle;
char gl_arg1[GL_ARGLEN+1];
char gl_arg2[GL_ARGLEN+1];
U8 gl_argptr = 0;
int gl_pair;
U8 gl_cmt = 0;
U8 gl_pen = 1;
...
U8 gl_dvx = 0;
U8 gl_dvy = 0;
U8 gl_rotate_chars = 0;
U8 gl_rotate_coords = 0;
U8 gl_symbol = 0;
U8 gl_pw[PEN_COUNT];
U8 gl_pwu;
U8 gl_charset = 0;
...
U8 gl_lt_rel;
double gl_lt_len;
double gl_newx, gl_newy;
U8 gl_rel_cmd = 0;
double gl_char_sx=0.75/100.0, gl_char_sy=1.5/100.0;
U8 gl_rel_char = 1;
double gl_x, gl_y;
double gl_cr_x, gl_cr_y;
double gl_textx, gl_texty;
double gl_spacex, gl_spacey;
double gl_extra_x, gl_extra_y;
double gl_text_ofsx, gl_text_ofsy;
double gl_lo_x, gl_lo_y;
int gl_lo_code;
int gl_char_num;
double gl_pos_tick = 0.5/100.0;
double gl_neg_tick = 0.5/100.0;
double VX = 8.0;
double VY = 8.0;

U8 rotate;
U8 pattern_list[MAX_PATTERN_LENGTH];
U8 pattern_length;
U8 line_pattern = 0;
int pattern_count = 0;
U8 temp_line_pattern;
COLOR initial_line_color;
COLOR temp_pen_color;

double u, du, X1, Y1, X2, Y2, X3, Y3, X4, Y4;
U8 DrawCmdActive;
U8 new_lbterm;
int pcl_row, pcl_col;
int pcl_encoding;
...
int pcl_run_start;
U8 pcl_last_bit;
U8 pcl_buf[PCL_BUF_LEN+1];
int pcl_index;

U8 gl_user_scaling = 0;
double gl_length, gl_width;
...
void draw_Wu_line(short X0, short Y0, short X1, short Y1)
{
   ...
   WeightingComplementMask = (unsigned short)NumLevels - 1;
...
void dot(int x, int y, U8 color)
{
   ...
   if (background)
      {
      if (color == 0) color = (U8)background;
...
void draw_line(int x1,int y1, int x2,int y2)
{
   if (antialias && (background == 0))
      draw_Wu_line((short)x1, (short)y1, (short)x2, (short)y2);
   else
...
U8 clipped_line(int x1,int y1, int x2,int y2)   // clipped line
{
   ...
   while ((c1 | c2) && (clip_count++ < 4))
      ...
      if (c & CS_TOP)     // clip to top FIRST
         {
         y = pVP->WinY1;
         x = x1 + (int)((dx * (long)(y-y1)) / dy);   // calc x for which y = VPminY
         }
      else if (c & CS_LEFT)
         {
         x = pVP->WinX1;
         y = y1 + (int)((dy * (long)(x-x1)) / dx);
         }
      else if (c & CS_RIGHT)
         {
         x = pVP->WinX2;
...
U8 vg_00[] PROGMEM = { 0xFF };
U8 vg_01[] PROGMEM = { 0x01,0x90,0xD0,0xE1,0xE6, 0x01,0x86,0x97,0xD7,0xE6, 0x22,
                        0x24,0xB5,0xC4, 0x4A
                      };
U8 vg_02[] PROGMEM = { 0x01,0x90,0xC3, 0x01,0xE1,0xE6, 0x01,0x86,0x97,0xD7,0xE6,
...
U8 vg_03[] PROGMEM = { 0x01,0x90,0xC3, 0x01,0xB4, 0x01,0xC1,0xC3, 0x01,0x83,0xA5,0xC3,
                        0x02,0xA4,0xC2, 0x02,0xC2, 0x03,0xB0,0xC1, 0x03,0xC3, 0x10,0x94,0xC1,
                        0x11,0xB3, 0x13,0xB1, 0x14,0xB4, 0x21,0xA6, 0x30,0xBC
                      };
U8 vg_04[] PROGMEM = { 0x03,0xB6,0xE3, 0x03,0xB0,0xE3, 0x03,0xE3, 0x12,0xC5, 0x12,0xD2,0xD4,
                        0x12,0x94,0xC1,0xC5, 0x13,0xB5,0xD3, 0x13,0xB1,0xD3, 0x14,0xD4,
                        0x21,0xD4, 0x21,0xC1, 0x21,0xA5,0xD2, 0x22,0xC4, 0x24,0xC2,
                        0x25,0xC5, 0x30,0xBE
                      };
U8 vg_05[] PROGMEM = { 0x03,0xC7, 0x03,0xD3,0xD4, 0x03,0x84,0xB7, 0x04,0xB1, 0x04,0xD4,
...
U8 vg_06[] PROGMEM = { 0x03,0xC7, 0x03,0xB0,0xE3,0xE5, 0x03,0xE3, 0x03,0x85,0xC1,0xC5,
...
U8 vg_07[] PROGMEM = { 0x23,0xB4,0xC3,0xC4, 0x23,0xB2,0xC3, 0x23,0xC3, 0x23,0xA4,0xB3,0xC4,
                        0x24,0xB5,0xC4, 0x24,0xC4, 0x32,0xBD
                      };
U8 vg_08[] PROGMEM = { 0x00,0xE0,0xE7, 0x00,0x87,0xE7, 0x01,0x90,0x92, 0x01,0xE1,
...
U8 vg_09[] PROGMEM = { 0x03,0xA1,0xC1,0xE3,0xE4, 0x03,0x84,0xA6,0xC6,0xEC };
U8 vg_0A[] PROGMEM = { 0x00,0xE0,0xE7, 0x00,0x87,0xE7, 0x23,0xB4,0xC3,0xC4, 0x23,0xB2,0xC3,
                        0x23,0xC3, 0x23,0xA4,0xB3,0xC4, 0x24,0xB5,0xC4, 0x24,0xC4,
                        0x32,0xBD
                      };
U8 vg_0B[] PROGMEM = { 0x04,0x93,0xB3,0xC4,0xC6, 0x04,0x86,0x97,0xB7,0xC6, 0x33,0xE0,0xE2,
                        0x40,0xE8
                      };
U8 vg_0C[] PROGMEM = { 0x11,0xA0,0xC0,0xD1,0xD3, 0x11,0x93,0xA4,0xC4,0xD3, 0x16,0xD6,
                        0x34,0xBF
                      };
U8 vg_0D[] PROGMEM = { 0x06,0x95,0xA5, 0x06,0x87,0x97,0xA6, 0x20,0xE0,0xE2, 0x20,0xA6,
                        0x22,0xEA
                      };
U8 vg_0E[] PROGMEM = { 0x05,0x94,0xA4, 0x05,0x86,0x96,0xA5, 0x20,0xE0,0xE6, 0x20,0xA5,
                        0x22,0xE2, 0x46,0xD5,0xE5, 0x46,0xC7,0xD7,0xEE
                      };
U8 vg_0F[] PROGMEM = { 0x00,0xA2, 0x03,0x93,0xB5,0xD3,0xE3, 0x06,0xA4, 0x13,0xB1,0xD3,
                        0x30,0xB1, 0x35,0xB6, 0x42,0xE0, 0x44,0xEE
                      };
U8 vg_10[] PROGMEM = { 0x00,0xB3, 0x00,0x86,0xB3, 0x01,0xA3, 0x01,0x91,0x95, 0x02,0xA4,
                        0x02,0xA2,0xA4, 0x03,0xB3, 0x04,0xA2, 0x04,0xA4, 0x05,0xA3,
                        0x05,0x9D
                      };
U8 vg_11[] PROGMEM = { 0x23,0xD6, 0x23,0xD0,0xD6, 0x23,0xD3, 0x32,0xD4, 0x32,0xD2,
                        0x32,0xB4,0xD2, 0x33,0xD5, 0x33,0xD1, 0x34,0xD4, 0x41,0xD1,
                        0x41,0xC5,0xDD
                      };
U8 vg_12[] PROGMEM = { 0x12,0xB0,0xD2, 0x15,0xB7,0xD5, 0x30,0xBF };
U8 vg_13[] PROGMEM = { 0x10,0x94, 0x16, 0x50,0xD4, 0x5E };
U8 vg_14[] PROGMEM = { 0x01,0x90,0xE0,0xE6, 0x01,0x82,0x93,0xB3, 0x30,0xBE };
U8 vg_15[] PROGMEM = { 0x06,0x97,0xB7,0xC6, 0x11,0xA2,0xB2,0xC3,0xC4, 0x11,0xA0,0xC0,0xD1,
                        0x13,0xA2, 0x13,0x94,0xA5,0xB5,0xC6, 0x35,0xCC
                      };
U8 vg_16[] PROGMEM = { 0x14,0xB6,0xD4,0xD6, 0x14,0xD4, 0x14,0x96,0xB4,0xD6, 0x15,0xA6,0xC4,0xD5,
                        0x15,0xA4,0xC6,0xD5, 0x15,0xD5, 0x16,0xD6, 0x24,0xA6, 0x34,0xB6,
                        0x44,0xCE
                      };
U8 vg_17[] PROGMEM = { 0x12,0xB0,0xD2, 0x14,0xB6,0xD4, 0x17,0xD7, 0x30,0xBE };
U8 vg_18[] PROGMEM = { 0x12,0xB0,0xD2, 0x30,0xBE };
U8 vg_19[] PROGMEM = { 0x14,0xB6,0xD4, 0x30,0xBE };
U8 vg_1A[] PROGMEM = { 0x03,0xD3, 0x31,0xD3, 0x35,0xDB };
U8 vg_1B[] PROGMEM = { 0x03,0xA5, 0x03,0xA1, 0x03,0xDB };
U8 vg_1C[] PROGMEM = { 0x02,0x85,0xDD };
U8 vg_1D[] PROGMEM = { 0x03,0xA5, 0x03,0xA1, 0x03,0xE3, 0x41,0xE3, 0x45,0xEB };
U8 vg_1E[] PROGMEM = { 0x04,0xB1,0xE4, 0x04,0xE4, 0x13,0xD3,0xD4, 0x13,0x94,0xB2,0xD4,
                        0x22,0xC4, 0x22,0xC2,0xC4, 0x22,0xA4,0xC2, 0x31,0xBC
                      };
U8 vg_1F[] PROGMEM = { 0x02,0xB5,0xE2, 0x02,0xE2, 0x12,0xB4,0xD2,0xD3, 0x12,0x93,0xD3,
                        0x22,0xC4, 0x22,0xA4,0xC2,0xC4, 0x24,0xC4, 0x32,0xBD
                      };
U8 vg_20[] PROGMEM = { 0xFF };
U8 vg_21[] PROGMEM = { 0x11,0xA0,0xB1,0xB3, 0x11,0xB3, 0x11,0xB1, 0x11,0x93,0xA4,0xB3,
                        0x12,0xB2, 0x13,0xB1, 0x13,0xB3, 0x20,0xA4, 0x2E
                      };
U8 vg_22[] PROGMEM = { 0x10,0x92, 0x40,0xCA };
U8 vg_23[] PROGMEM = { 0x02,0xD2, 0x04,0xD4, 0x10,0x96, 0x40,0xCE };
U8 vg_24[] PROGMEM = { 0x02,0x93,0xB3,0xC4, 0x02,0x91,0xC1, 0x05,0xB5,0xC4, 0x20,0xA1,
                        0x25,0xAE
                      };
U8 vg_25[] PROGMEM = { 0x01,0x91,0x92, 0x01,0x82,0x92, 0x06,0xD1, 0x45,0xD5,0xD6,
                        0x45,0xC6,0xDE
                      };
U8 vg_26[] PROGMEM = { 0x04,0xA2,0xB2,0xD4,0xE3, 0x04,0x85,0x96,0xC6,0xD5,0xE6,
                        0x11,0xA2, 0x11,0xA0,0xB0,0xC1, 0x32,0xC1, 0x54,0xDD
                      };
U8 vg_27[] PROGMEM = { 0x02,0x91, 0x10,0x99 };
U8 vg_28[] PROGMEM = { 0x12,0xB0, 0x12,0x94,0xBE };
U8 vg_29[] PROGMEM = { 0x10,0xB2,0xB4, 0x16,0xBC };
U8 vg_2A[] PROGMEM = { 0x03,0xE3, 0x11,0xD5, 0x15,0xD1, 0x30,0xBE };
U8 vg_2B[] PROGMEM = { 0x03,0xC3, 0x21,0xAD };
U8 vg_2C[] PROGMEM = { 0x17,0xA6, 0x25,0xAE };
U8 vg_2D[] PROGMEM = { 0x03,0xCB };
U8 vg_2E[] PROGMEM = { 0x25,0xAE };
U8 vg_2F[] PROGMEM = { 0x05,0xD8 };
U8 vg_30[] PROGMEM = { 0x01,0x90,0xC0,0xD1,0xD5, 0x01,0x85,0x96,0xC6,0xD5, 0x05,0xD9 };
U8 vg_31[] PROGMEM = { 0x11,0xA0,0xA6, 0x16,0xBE };
U8 vg_32[] PROGMEM = { 0x01,0x90,0xB0,0xC1,0xC2, 0x05,0xA3,0xB3,0xC2, 0x05,0x86,0xCE };
U8 vg_33[] PROGMEM = { 0x01,0x90,0xB0,0xC1,0xC2, 0x05,0x96,0xB6,0xC5, 0x23,0xB3,0xC4,0xC5,
                        0x33,0xCA
                      };
U8 vg_34[] PROGMEM = { 0x03,0xB0,0xC0,0xC6, 0x03,0x84,0xDC };
U8 vg_35[] PROGMEM = { 0x00,0xC0, 0x00,0x82,0xB2,0xC3,0xC5, 0x05,0x96,0xB6,0xCD };
U8 vg_36[] PROGMEM = { 0x02,0xA0,0xB0, 0x02,0x85,0x96,0xB6,0xC5, 0x03,0xB3,0xC4,0xCD };
U8 vg_37[] PROGMEM = { 0x00,0xC0,0xC2, 0x00,0x81, 0x24,0xC2, 0x24,0xAE };
U8 vg_38[] PROGMEM = { 0x01,0x90,0xB0,0xC1,0xC2, 0x01,0x82,0x93,0xB3,0xC4,0xC5,
                        0x04,0x93, 0x04,0x85,0x96,0xB6,0xC5, 0x33,0xCA
                      };
U8 vg_39[] PROGMEM = { 0x01,0x90,0xB0,0xC1,0xC4, 0x01,0x82,0x93,0xC3, 0x16,0xA6,0xCC };
U8 vg_3A[] PROGMEM = { 0x21,0xA2, 0x25,0xAE };
U8 vg_3B[] PROGMEM = { 0x17,0xA6, 0x21,0xA2, 0x25,0xAE };
U8 vg_3C[] PROGMEM = { 0x03,0xB6, 0x03,0xB8 };
U8 vg_3D[] PROGMEM = { 0x02,0xC2, 0x05,0xCD };
U8 vg_3E[] PROGMEM = { 0x10,0xC3, 0x16,0xCB };
U8 vg_3F[] PROGMEM = { 0x01,0x90,0xB0,0xC1,0xC2, 0x24,0xC2, 0x24,0xA5, 0x2F };
U8 vg_40[] PROGMEM = { 0x01,0x90,0xC0,0xD1,0xD4, 0x01,0x85,0x96,0xB6, 0x32,0xB4,0xDC };
U8 vg_41[] PROGMEM = { 0x02,0xA0,0xC2,0xC6, 0x02,0x86, 0x04,0xCC };
U8 vg_42[] PROGMEM = { 0x00,0xB0,0xC1,0xC2, 0x00,0x86,0xB6,0xC5, 0x03,0xB3,0xC4,0xC5,
                        0x33,0xCA
                      };
U8 vg_43[] PROGMEM = { 0x02,0xA0,0xC0,0xD1, 0x02,0x84,0xA6,0xC6,0xDD };
U8 vg_44[] PROGMEM = { 0x00,0xA0,0xC2,0xC4, 0x00,0x86,0xA6,0xCC };
U8 vg_45[] PROGMEM = { 0x10,0xD0, 0x10,0x96,0xD6, 0x13,0xBB };
U8 vg_46[] PROGMEM = { 0x10,0xD0, 0x10,0x96, 0x13,0xBB };
U8 vg_47[] PROGMEM = { 0x02,0xA0,0xC0,0xD1, 0x02,0x84,0xA6,0xD6, 0x44,0xD4,0xDE };
U8 vg_48[] PROGMEM = { 0x00,0x86, 0x03,0xC3, 0x40,0xCE };
U8 vg_49[] PROGMEM = { 0x10,0xB0, 0x16,0xB6, 0x20,0xAE };
U8 vg_4A[] PROGMEM = { 0x04,0x85,0x96,0xB6,0xC5, 0x30,0xD0, 0x40,0xCD };
U8 vg_4B[] PROGMEM = { 0x00,0x86, 0x03,0xA3,0xC5,0xC6, 0x23,0xC1, 0x40,0xC9 };
U8 vg_4C[] PROGMEM = { 0x10,0x96,0xDE };
U8 vg_4D[] PROGMEM = { 0x00,0xB3,0xE0,0xE6, 0x00,0x86, 0x33,0xBC };
U8 vg_4E[] PROGMEM = { 0x00,0xD5, 0x00,0x86, 0x50,0xDE };
U8 vg_4F[] PROGMEM = { 0x02,0xA0,0xB0,0xD2,0xD4, 0x02,0x84,0xA6,0xB6,0xDC };
U8 vg_50[] PROGMEM = { 0x10,0xC0,0xD1,0xD2, 0x10,0x96, 0x13,0xC3,0xDA };
U8 vg_51[] PROGMEM = { 0x01,0x90,0xB0,0xC1,0xC5, 0x01,0x85,0x96,0xB6,0xC5, 0x24,0xDF };
U8 vg_52[] PROGMEM = { 0x10,0xC0,0xD1,0xD2, 0x10,0x96, 0x13,0xC3,0xD2, 0x33,0xD5,0xDE };
U8 vg_53[] PROGMEM = { 0x01,0x90,0xB0,0xC1, 0x01,0x82,0x93,0xB3,0xC4,0xC5, 0x05,0x96,0xB6,0xCD };
U8 vg_54[] PROGMEM = { 0x00,0xC0, 0x20,0xAE };
U8 vg_55[] PROGMEM = { 0x00,0x85,0x96,0xB6,0xC5, 0x40,0xCD };
U8 vg_56[] PROGMEM = { 0x00,0x84,0xA6,0xC4, 0x40,0xCC };
U8 vg_57[] PROGMEM = { 0x00,0x86,0xB3,0xE6, 0x32,0xB3, 0x60,0xEE };
U8 vg_58[] PROGMEM = { 0x00,0x81,0xD6, 0x06,0xD1, 0x50,0xD9 };
U8 vg_59[] PROGMEM = { 0x00,0x82,0xA4,0xC2, 0x16,0xB6, 0x24,0xA6, 0x40,0xCA };
U8 vg_5A[] PROGMEM = { 0x10,0xD0,0xD1, 0x15,0xD1, 0x15,0x96,0xDE };
U8 vg_5B[] PROGMEM = { 0x10,0xB0, 0x10,0x96,0xBE };
U8 vg_5C[] PROGMEM = { 0x00,0xEE };
U8 vg_5D[] PROGMEM = { 0x10,0xB0,0xB6, 0x16,0xBE };
U8 vg_5E[] PROGMEM = { 0x03,0xB0,0xEB };
U8 vg_5F[] PROGMEM = { 0x07,0xEF };
U8 vg_60[] PROGMEM = { 0x20,0xA1,0xBA };
U8 vg_61[] PROGMEM = { 0x05,0x96,0xB6,0xC5,0xD6, 0x05,0x94,0xC4, 0x12,0xB2,0xC3,0xCD };
U8 vg_62[] PROGMEM = { 0x06,0x95,0xA6,0xC6,0xD5, 0x10,0x95, 0x13,0xC3,0xD4,0xDD };
U8 vg_63[] PROGMEM = { 0x03,0x92,0xB2,0xC3, 0x03,0x85,0x96,0xB6,0xCD };
U8 vg_64[] PROGMEM = { 0x04,0x93,0xC3, 0x04,0x85,0x96,0xB6,0xC5,0xD6, 0x40,0xCD };
U8 vg_65[] PROGMEM = { 0x03,0x92,0xB2,0xC3,0xC4, 0x03,0x85,0x96,0xB6, 0x04,0xCC };
U8 vg_66[] PROGMEM = { 0x03,0xA3, 0x06,0xA6, 0x11,0xA0,0xB0,0xC1, 0x11,0x9E };
U8 vg_67[] PROGMEM = { 0x03,0x92,0xB2,0xC3,0xD2, 0x03,0x84,0x95,0xC5, 0x07,0xB7,0xC6,
                        0x43,0xCE
                      };
U8 vg_68[] PROGMEM = { 0x10,0x96, 0x14,0xB2,0xC2,0xD3,0xDE };
U8 vg_69[] PROGMEM = { 0x12,0xA2,0xA6, 0x16,0xB6, 0x28 };
U8 vg_6A[] PROGMEM = { 0x05,0x86,0x97,0xB7,0xC6, 0x40, 0x42,0xCE };
U8 vg_6B[] PROGMEM = { 0x10,0x96, 0x14,0xB4,0xD6, 0x34,0xDA };
U8 vg_6C[] PROGMEM = { 0x10,0xA0,0xA6, 0x16,0xBE };
U8 vg_6D[] PROGMEM = { 0x02,0x86, 0x04,0xA2,0xB3,0xC2,0xD3,0xD6, 0x33,0xBD };
U8 vg_6E[] PROGMEM = { 0x02,0x86, 0x04,0xA2,0xB2,0xC3,0xCE };
U8 vg_6F[] PROGMEM = { 0x03,0x92,0xB2,0xC3,0xC5, 0x03,0x85,0x96,0xB6,0xCD };
U8 vg_70[] PROGMEM = { 0x02,0x93,0xA2,0xC2,0xD3,0xD4, 0x07,0xA7, 0x13,0x97, 0x15,0xC5,0xDC };
U8 vg_71[] PROGMEM = { 0x03,0x92,0xB2,0xC3,0xD2, 0x03,0x84,0x95,0xC5, 0x37,0xD7,
                        0x43,0xCF
                      };
U8 vg_72[] PROGMEM = { 0x02,0x93,0xA2,0xB2,0xC3, 0x06,0xA6, 0x13,0x9E };
U8 vg_73[] PROGMEM = { 0x03,0x94,0xB4,0xC5, 0x03,0x92,0xC2, 0x06,0xB6,0xCD };
U8 vg_74[] PROGMEM = { 0x12,0xC2, 0x20,0xA5,0xB6,0xCD };
U8 vg_75[] PROGMEM = { 0x02,0x85,0x96,0xB6,0xC5,0xD6, 0x42,0xCD };
U8 vg_76[] PROGMEM = { 0x02,0x84,0xA6,0xC4, 0x42,0xCC };
U8 vg_77[] PROGMEM = { 0x02,0x85,0x96,0xB4,0xD6,0xE5, 0x33,0xB4, 0x62,0xED };
U8 vg_78[] PROGMEM = { 0x02,0xA4,0xB4,0xD6, 0x06,0xA4, 0x34,0xDA };
U8 vg_79[] PROGMEM = { 0x02,0x84,0x95,0xC5, 0x07,0xB7,0xC6, 0x42,0xCE };
U8 vg_7A[] PROGMEM = { 0x02,0xC2, 0x06,0xC2, 0x06,0xCE };
U8 vg_7B[] PROGMEM = { 0x03,0x93,0xA4,0xA5,0xB6,0xC6, 0x13,0xA2, 0x21,0xB0,0xC0,
                        0x21,0xAA
                      };
U8 vg_7C[] PROGMEM = { 0x30,0xB2, 0x34,0xBE };
U8 vg_7D[] PROGMEM = { 0x00,0x90,0xA1,0xA2,0xB3,0xC3, 0x06,0x96,0xA5, 0x24,0xB3,
                        0x24,0xAD
                      };
U8 vg_7E[] PROGMEM = { 0x01,0x90,0xA0,0xB1,0xC1,0xD8 };
U8 vg_7F[] PROGMEM = { 0x04,0xB1,0xE4,0xE6, 0x04,0x86,0xEE };
U8 vg_80[] PROGMEM = { 0xFF }; 
U8 vg_81[] PROGMEM = { 0x01, 0x03,0x85,0x96,0xB6,0xC5,0xD6, 0x41, 0x43,0xCD };
U8 vg_82[] PROGMEM = { 0x03,0x92,0xB2,0xC3,0xC4, 0x03,0x85,0x96,0xB6, 0x04,0xC4,
                        0x30,0xC8
                      };
U8 vg_83[] PROGMEM = { 0x01,0x90,0xD0,0xE1, 0x15,0xA6,0xC6,0xD5,0xE6, 0x15,0xA4,0xD4,
                        0x22,0xC2,0xD3,0xDD
                      };
U8 vg_84[] PROGMEM = { 0x05,0x96,0xB6,0xC5,0xD6, 0x05,0x94,0xC4, 0x10, 0x12,0xB2,0xC3,0xC5, 0x48 };
U8 vg_85[] PROGMEM = { 0x00,0x90, 0x05,0x96,0xB6,0xC5,0xD6, 0x05,0x94,0xC4, 0x12,0xB2,0xC3,0xCD };
U8 vg_86[] PROGMEM = { 0x05,0x96,0xB6,0xC5,0xD6, 0x05,0x94,0xC4, 0x12,0xB2,0xC3,0xC5,
                        0x20,0xB0,0xB1, 0x20,0xA1,0xB9
                      };
U8 vg_87[] PROGMEM = { 0x03,0x92,0xC2,0xD3, 0x03,0x84,0x95,0xC5, 0x07,0x97,0xBD };
U8 vg_88[] PROGMEM = { 0x01,0x90,0xD0,0xE1, 0x13,0xA2,0xC2,0xD3,0xD4, 0x13,0x95,0xA6,0xC6,
                        0x14,0xDC
                      };
U8 vg_89[] PROGMEM = { 0x00, 0x03,0x92,0xB2,0xC3,0xC4, 0x03,0x85,0x96,0xB6, 0x04,0xC4, 0x48 };
U8 vg_8A[] PROGMEM = { 0x00,0xA0, 0x03,0x92,0xB2,0xC3,0xC4, 0x03,0x85,0x96,0xB6,
                        0x04,0xCC
                      };
U8 vg_8B[] PROGMEM = { 0x00, 0x12,0xA2,0xA6, 0x16,0xB6, 0x48 };
U8 vg_8C[] PROGMEM = { 0x01,0x90,0xC0,0xD1, 0x22,0xB2,0xB6, 0x26,0xCE };
U8 vg_8D[] PROGMEM = { 0x00,0xA0, 0x12,0xA2,0xA6, 0x16,0xBE };
U8 vg_8E[] PROGMEM = { 0x00, 0x03,0xA1,0xC3,0xC6, 0x03,0x86, 0x04,0xC4, 0x48 };
U8 vg_8F[] PROGMEM = { 0x04,0x93,0xB3,0xC4,0xC6, 0x04,0x86,0x05,0xC5, 0x20,0xB0,0xB1,
                        0x20,0xA1,0xB9
                      };
U8 vg_90[] PROGMEM = { 0x12,0xC2, 0x12,0x96,0xC6, 0x14,0xB4, 0x20,0xC8 };
U8 vg_91[] PROGMEM = { 0x15,0xA4,0xE4, 0x15,0x96,0xE6, 0x22,0xE2, 0x42,0xCE };
U8 vg_92[] PROGMEM = { 0x02,0xA0,0xE0, 0x02,0x86, 0x03,0xD3, 0x30,0xB6,0xEE };
U8 vg_93[] PROGMEM = { 0x01,0x90,0xB0,0xC1, 0x04,0x93,0xB3,0xC4,0xC5, 0x04,0x85,0x96,0xB6,0xCD };
U8 vg_94[] PROGMEM = { 0x01, 0x04,0x93,0xB3,0xC4,0xC5, 0x04,0x85,0x96,0xB6,0xC5, 0x49 };
U8 vg_95[] PROGMEM = { 0x01,0xA1, 0x04,0x93,0xB3,0xC4,0xC5, 0x04,0x85,0x96,0xB6,0xCD };
U8 vg_96[] PROGMEM = { 0x01,0x90,0xB0,0xC1, 0x03,0x85,0x96,0xB6,0xC5,0xD6, 0x43,0xCD };
U8 vg_97[] PROGMEM = { 0x01,0xA1, 0x03,0x85,0x96,0xB6,0xC5,0xD6, 0x43,0xCD };
U8 vg_98[] PROGMEM = { 0x01, 0x03,0x84,0x95,0xC5, 0x07,0xB7,0xC6, 0x41, 0x43,0xCE };
U8 vg_99[] PROGMEM = { 0x00, 0x13,0xB1,0xD3,0xD4, 0x13,0x94,0xB6,0xD4, 0x68 };
U8 vg_9A[] PROGMEM = { 0x00, 0x02,0x85,0x96,0xB6,0xC5, 0x40, 0x42,0xCD };
U8 vg_9B[] PROGMEM = { 0x03,0x92,0xD2, 0x03,0x84,0x95,0xD5, 0x30,0xBF };
U8 vg_9C[] PROGMEM = { 0x03,0xA3, 0x06,0xC6,0xD5, 0x11,0xA0,0xB0,0xC1,0xC2, 0x11,0x9E };
U8 vg_9D[] PROGMEM = { 0x00,0x81,0xA3,0xC1, 0x04,0xC4, 0x06,0xC6, 0x12, 0x23,0xA7,
                        0x32, 0x40,0xC9
                      };
U8 vg_9E[] PROGMEM = { 0x00,0xB0,0xC1,0xC2, 0x00,0x87, 0x03,0xB3,0xC2, 0x54,0xF4,
                        0x63,0xE6,0xF7, 0xFF
                      };
U8 vg_9F[] PROGMEM = { 0x06,0x97,0xA7,0xB6, 0x23,0xC3, 0x31,0xC0,0xD0,0xE1, 0x31,0xBE };
U8 vg_A0[] PROGMEM = { 0x05,0x96,0xB6,0xC5,0xD6, 0x05,0x94,0xC4, 0x12,0xB2,0xC3,0xC5,
                        0x20,0xC8
                      };
U8 vg_A1[] PROGMEM = { 0x12,0xA2,0xA6, 0x16,0xB6, 0x20,0xC8 };
U8 vg_A2[] PROGMEM = { 0x04,0x93,0xB3,0xC4,0xC5, 0x04,0x85,0x96,0xB6,0xC5, 0x21,0xC9 };
U8 vg_A3[] PROGMEM = { 0x03,0x85,0x96,0xB6,0xC5,0xD6, 0x21,0xC1, 0x43,0xCD };
U8 vg_A4[] PROGMEM = { 0x01,0x90,0xA0,0xB1,0xC1,0xD0, 0x03,0x86, 0x04,0x93,0xB3,0xC4,0xCE };
U8 vg_A5[] PROGMEM = { 0x00,0xC0, 0x02,0xC6, 0x02,0x86, 0x42,0xCE };
U8 vg_A6[] PROGMEM = { 0x11,0xA0,0xC0,0xC3, 0x11,0x92,0xA3,0xD3, 0x15,0xDD };
U8 vg_A7[] PROGMEM = { 0x11,0xA0,0xB0,0xC1,0xC2, 0x11,0x92,0xA3,0xB3,0xC2, 0x15,0xCD };
U8 vg_A8[] PROGMEM = { 0x15,0xA6,0xC6,0xD5, 0x15,0xB3, 0x30, 0x32,0xBB };
U8 vg_A9[] PROGMEM = { 0x03,0xC3, 0x03,0x8D };
U8 vg_AA[] PROGMEM = { 0x03,0xC3,0xCD };
U8 vg_AB[] PROGMEM = { 0x00,0x82, 0x05,0xD0, 0x34,0xC3,0xD3,0xE4, 0x46,0xE4, 0x46,0xC7,0xEF };
U8 vg_AC[] PROGMEM = { 0x00,0x82, 0x05,0xD0, 0x44,0xE2,0xE7, 0x44,0xC5,0xED };
U8 vg_AD[] PROGMEM = { 0x13,0xA2,0xB3,0xB5, 0x13,0xB5, 0x13,0xB3, 0x13,0x95,0xA6,0xB5,
                        0x14,0xB4, 0x15,0xB3, 0x15,0xB5, 0x20, 0x22,0xAE
                      };
U8 vg_AE[] PROGMEM = { 0x03,0xA5, 0x03,0xA1, 0x43,0xE5, 0x43,0xE9 };
U8 vg_AF[] PROGMEM = { 0x01,0xA3, 0x05,0xA3, 0x41,0xE3, 0x45,0xEB };
U8 vg_B0[] PROGMEM = { 0x01, 0x03, 0x05, 0x07, 0x20, 0x22, 0x24, 0x26, 0x41, 0x43, 0x45,
                        0x47, 0x60, 0x62, 0x64, 0x6E
                      };
U8 vg_B1[] PROGMEM = { 0x01,0xE7,0xF6, 0x01,0x90,0xF6, 0x03,0xC7,0xF4, 0x03,0xB0,0xF4,
                        0x05,0xA7,0xF2, 0x05,0xD0,0xF2, 0x07,0xF8
                      };
U8 vg_B2[] PROGMEM = { 0x01,0x91,0xA2,0xB2,0xC3,0xD3,0xE4,0xF4, 0x03,0x93,0xA4,0xB4,0xC5,0xD5,0xE6,0xF6,
...
U8 vg_B3[] PROGMEM = { 0x30,0xBF };
U8 vg_B4[] PROGMEM = { 0x04,0xB4, 0x30,0xBF };
U8 vg_B5[] PROGMEM = { 0x02,0xB2, 0x04,0xB4, 0x30,0xBF };
U8 vg_B6[] PROGMEM = { 0x04,0xB4, 0x30,0xB7, 0x60,0xEF };
U8 vg_B7[] PROGMEM = { 0x04,0xE4,0xE7, 0x34,0xBF };
U8 vg_B8[] PROGMEM = { 0x02,0xB2,0xB7, 0x04,0xBC };
U8 vg_B9[] PROGMEM = { 0x02,0xB2, 0x04,0xB4,0xB7, 0x30,0xB2, 0x60,0xEF };
U8 vg_BA[] PROGMEM = { 0x30,0xB7, 0x60,0xEF };
U8 vg_BB[] PROGMEM = { 0x02,0xE2,0xE7, 0x04,0xB4,0xBF };
U8 vg_BC[] PROGMEM = { 0x02,0xB2, 0x04,0xE4, 0x30,0xB2, 0x60,0xEC };
U8 vg_BD[] PROGMEM = { 0x04,0xE4, 0x30,0xB4, 0x60,0xEC };
U8 vg_BE[] PROGMEM = { 0x02,0xB2, 0x04,0xB4, 0x30,0xBC };
U8 vg_BF[] PROGMEM = { 0x04,0xB4,0xBF };
U8 vg_C0[] PROGMEM = { 0x30,0xB4,0xFC };
U8 vg_C1[] PROGMEM = { 0x04,0xF4, 0x30,0xBC };
U8 vg_C2[] PROGMEM = { 0x04,0xF4, 0x34,0xBF };
U8 vg_C3[] PROGMEM = { 0x30,0xB7, 0x34,0xFC };
U8 vg_C4[] PROGMEM = { 0x04,0xFC };
U8 vg_C5[] PROGMEM = { 0x04,0xF4, 0x30,0xBF };
U8 vg_C6[] PROGMEM = { 0x30,0xB7, 0x32,0xF2, 0x34,0xFC };
U8 vg_C7[] PROGMEM = { 0x30,0xB7, 0x60,0xE7, 0x64,0xFC };
U8 vg_C8[] PROGMEM = { 0x30,0xB4,0xF4, 0x60,0xE2,0xFA };
U8 vg_C9[] PROGMEM = { 0x32,0xF2, 0x32,0xB7, 0x64,0xF4, 0x64,0xEF };
U8 vg_CA[] PROGMEM = { 0x02,0xB2, 0x04,0xF4, 0x30,0xB2, 0x60,0xE2,0xFA };
U8 vg_CB[] PROGMEM = { 0x02,0xF2, 0x04,0xB4,0xB7, 0x64,0xF4, 0x64,0xEF };
U8 vg_CC[] PROGMEM = { 0x30,0xB7, 0x60,0xE2,0xF2, 0x64,0xF4, 0x64,0xEF };
U8 vg_CD[] PROGMEM = { 0x02,0xF2, 0x04,0xFC };
U8 vg_CE[] PROGMEM = { 0x02,0xB2, 0x04,0xB4,0xB7, 0x30,0xB2, 0x60,0xE2,0xF2, 0x64,0xF4,
                        0x64,0xEF
                      };
U8 vg_CF[] PROGMEM = { 0x02,0xF2, 0x04,0xF4, 0x40,0xCA };
U8 vg_D0[] PROGMEM = { 0x04,0xF4, 0x30,0xB4, 0x60,0xEC };
U8 vg_D1[] PROGMEM = { 0x02,0xF2, 0x04,0xF4, 0x34,0xBF };
U8 vg_D2[] PROGMEM = { 0x04,0xF4, 0x34,0xB7, 0x64,0xEF };
U8 vg_D3[] PROGMEM = { 0x30,0xB4,0xF4, 0x60,0xEC };
U8 vg_D4[] PROGMEM = { 0x30,0xB4,0xF4, 0x32,0xFA };
U8 vg_D5[] PROGMEM = { 0x32,0xF2, 0x32,0xB7, 0x34,0xFC };
U8 vg_D6[] PROGMEM = { 0x34,0xF4, 0x34,0xB7, 0x64,0xEF };
U8 vg_D7[] PROGMEM = { 0x04,0xF4, 0x30,0xB7, 0x60,0xEF };
U8 vg_D8[] PROGMEM = { 0x02,0xF2, 0x04,0xF4, 0x30,0xBF };
U8 vg_D9[] PROGMEM = { 0x04,0xB4, 0x30,0xBC };
U8 vg_DA[] PROGMEM = { 0x34,0xF4, 0x34,0xBF };
U8 vg_DB[] PROGMEM = { 0x00,0xF0,0xF7, 0x00,0x87,0xF7, 0x01,0xF1, 0x02,0xF2, 0x03,0xF3,
                        0x04,0xF4, 0x05,0xF5, 0x06,0xF6, 0x10,0x97, 0x20,0xA7, 0x30,0xB7,
                        0x40,0xC7, 0x50,0xD7, 0x60,0xEF
                      };
U8 vg_DC[] PROGMEM = { 0x04,0xF4,0xF7, 0x04,0x87,0xF7, 0x05,0xF5, 0x06,0xF6, 0x14,0x97,
                        0x24,0xA7, 0x34,0xB7, 0x44,0xC7, 0x54,0xD7, 0x64,0xEF
                      };
U8 vg_DD[] PROGMEM = { 0x00,0xB0,0xB7, 0x00,0x87,0xB7, 0x01,0xB1, 0x02,0xB2, 0x03,0xB3,
                        0x04,0xB4, 0x05,0xB5, 0x06,0xB6, 0x10,0x97, 0x20,0xAF
                      };
U8 vg_DE[] PROGMEM = { 0x40,0xF0,0xF7, 0x40,0xC7,0xF7, 0x41,0xF1, 0x42,0xF2, 0x43,0xF3,
                        0x44,0xF4, 0x45,0xF5, 0x46,0xF6, 0x50,0xD7, 0x60,0xEF
                      };
U8 vg_DF[] PROGMEM = { 0x00,0xF0,0xF3, 0x00,0x83,0xF3, 0x01,0xF1, 0x02,0xF2, 0x10,0x93,
                        0x20,0xA3, 0x30,0xB3, 0x40,0xC3, 0x50,0xD3, 0x60,0xEB
                      };
U8 vg_E0[] PROGMEM = { 0x03,0x92,0xA2,0xB3,0xC3,0xD2, 0x03,0x85,0x96,0xA6,0xB5,0xC5,0xD6,
                        0x33,0xBD
                      };
U8 vg_E1[] PROGMEM = { 0x02,0x91,0xB1,0xC2, 0x02,0x87, 0x03,0xB3,0xC4, 0x05,0xB5,0xC4,
                        0x33,0xCA
                      };
U8 vg_E2[] PROGMEM = { 0x01,0xC1,0xC2, 0x01,0x8E };
U8 vg_E3[] PROGMEM = { 0x02,0xD2, 0x12,0x96, 0x42,0xCE };
U8 vg_E4[] PROGMEM = { 0x00,0xC0,0xC1, 0x00,0x81,0xA3, 0x05,0xA3, 0x05,0x86,0xC6,
                        0x45,0xCE
                      };
U8 vg_E5[] PROGMEM = { 0x03,0x92,0xD2, 0x03,0x85,0x96,0xA6,0xB5, 0x32,0xBD };
U8 vg_E6[] PROGMEM = { 0x07,0xA5,0xC5,0xD4, 0x11,0x94,0xA5, 0x51,0xDC };
U8 vg_E7[] PROGMEM = { 0x02,0x91,0xA1,0xB2,0xD2,0xE1, 0x32,0xBE };
U8 vg_E8[] PROGMEM = { 0x03,0x92,0xB2,0xC3,0xC4, 0x03,0x84,0x95,0xB5,0xC4, 0x10,0xB0,
                        0x17,0xB7, 0x20,0xAF
                      };
U8 vg_E9[] PROGMEM = { 0x02,0xA0,0xB0,0xD2,0xD4, 0x02,0x84,0xA6,0xB6,0xD4, 0x03,0xDB };
U8 vg_EA[] PROGMEM = { 0x02,0xA0,0xB0,0xD2,0xD3, 0x02,0x83,0x94,0x96, 0x06,0x96,
                        0x44,0xD3, 0x44,0xC6,0xDE
                      };
U8 vg_EB[] PROGMEM = { 0x04,0x93,0xC3, 0x04,0x85,0x96,0xB6,0xC5, 0x31,0xC2,0xC5,
                        0x31,0xC0,0xD8
                      };
U8 vg_EC[] PROGMEM = { 0x03,0x92,0xA2,0xB3,0xC2,0xD2,0xE3,0xE4, 0x03,0x84,0x95,0xA5,0xB4,0xC5,0xD5,0xE4,
                        0x33,0xBC
                      };
U8 vg_ED[] PROGMEM = { 0x02,0x91,0xD1,0xE2,0xE4, 0x02,0x84,0x95,0xD5,0xE4, 0x06,0xE8 };
U8 vg_EE[] PROGMEM = { 0x02,0xA0,0xB0, 0x02,0x84,0xA6,0xB6, 0x03,0xBB };
U8 vg_EF[] PROGMEM = { 0x01,0x90,0xB0,0xC1,0xC6, 0x01,0x8E };
U8 vg_F0[] PROGMEM = { 0x11,0xD1, 0x13,0xD3, 0x15,0xDD };
U8 vg_F1[] PROGMEM = { 0x12,0xD2, 0x16,0xD6, 0x30,0xBC };
U8 vg_F2[] PROGMEM = { 0x06,0xC6, 0x10,0xB2, 0x14,0xBA };
U8 vg_F3[] PROGMEM = { 0x06,0xC6, 0x12,0xB4, 0x12,0xB8 };
U8 vg_F4[] PROGMEM = { 0x31,0xC0,0xD0,0xE1,0xE2, 0x31,0xBF };
U8 vg_F5[] PROGMEM = { 0x05,0x86,0x97,0xA7,0xB6, 0x30,0xBE };
U8 vg_F6[] PROGMEM = { 0x13,0xD3, 0x30,0xB1, 0x35,0xBE };
U8 vg_F7[] PROGMEM = { 0x02,0x91,0xA1,0xB2,0xC2,0xD1, 0x05,0x94,0xA4,0xB5,0xC5,0xDC };
U8 vg_F8[] PROGMEM = { 0x11,0xA0,0xB0,0xC1,0xC2, 0x11,0x92,0xA3,0xB3,0xCA };
U8 vg_F9[] PROGMEM = { 0x33,0xBC };
U8 vg_FA[] PROGMEM = { 0x3C };
U8 vg_FB[] PROGMEM = { 0x15,0xB7,0xC7, 0x40,0xE0, 0x40,0xCF };
U8 vg_FC[] PROGMEM = { 0x00,0x91,0xA0,0xB0,0xC1,0xC4, 0x11,0x9C };
U8 vg_FD[] PROGMEM = { 0x01,0x90,0xA0,0xB1, 0x04,0xB1, 0x04,0xBC };
U8 vg_FE[] PROGMEM = { 0x22,0xC4, 0x22,0xC2,0xC5, 0x22,0xA5,0xC3, 0x23,0xC5, 0x23,0xC3,
                        0x24,0xC2, 0x24,0xC4, 0x25,0xC5, 0x32,0xBD
                      };
U8 vg_FF[] PROGMEM = { 0x01,0xD1,0xD6, 0x01,0x86,0xDE };
...
void vchar_char(int xoffset,int yoffset, int c)  // draw a vector character
{
   int x1,y1, x2,y2, x,y;
   U8 VByte;
   U8 *VIndex;  // char gen table offset
   int xslant;
   int dl;  //uuuu
   int dlc;
   int dl_stroke;
...
#ifdef CPU_CLOCK     // kluge for identifying MegaDonkey compile
   sx=(int)vx; sy=(int)vy;                  // technically correct
...
//vy = sy;
   ...
   vchar_set_fontsize((U8)sx, (U8)sy);
...
U8 gl_label_char(int data) //uuuu
{
...
#ifdef BUFFER_LABELS
unsigned char gl_label[BUFFER_LABELS+1];
int gl_label_index;
U8 gl_save_only = 0;  // print/store label flag

U8 gl_buffer_label(U8 data)
{
   if (data)
      {
...
void gl_print_buffer(unsigned char *buf, int do_lo)
{
   int i, j;
   int gl_label_index;
...
///gl_spacex /= 32.0;
   ...
   if (1 || do_lo)
      {
      concat = (U8)gl_set_lo(0, do_lo);  //llll
      }
   else
      {
      gl_lo_x = gl_lo_y = 0.0;
   ...
   gl_label_index = (int)strlen((char *) buf);
   gl_label_start = 0;
...
//printf("%c", buf[i]);
      if ((buf[i] == 0x0A) || (i == (gl_label_index - 1)))   // label end-of_line;
         {
//printf("\nlen2:%d\n", len);
         if (1 || do_lo) concat = (U8)gl_set_lo((double) len, do_lo); // llll
...
U8 lt0[] PM = { 0 };          // line type 0 is special
U8 lt1[] PM = { 2, 0,100 };
U8 lt2[] PM = { 2, 50,50 };
U8 lt3[] PM = { 2, 70,30 };
U8 lt4[] PM = { 4, 80,10, 0,10 };
U8 lt5[] PM = { 4, 70,10, 10,10 };
U8 lt6[] PM = { 6, 50,10, 10,10, 10,10 };
U8 lt7[] PM = { 6, 70,10,  0,10,  0,10 };
U8 lt8[] PM = { 8, 50,10,  0,10, 10,10, 0,10 };
...
U8 gl_user_linetype;
U8 gl_user_pattern[USER_LINE_TYPES+1][MAX_PATTERN_LENGTH+1];
#endif
...
void gl_setup_pattern()
{
   ...
   for (i=1; i<=pattern_length; i++)   // scale run length percenatages into pixel counts
      {
      pat_val = (U8)((d * (double) lpat[i]) / 100.0);
...
U8 gl_poly_number = 0;       // if non-zero we are defining a polygon
...
U8 poly_closed = 1;          // used to make first move in polygon with pen up
...
U8 gl_add_poly(double x,double y, U8 flags)
{
...
////  (gl_poly_buf[gl_poly_start].flags & (~POLY_PENDOWN)) | 0x20
      (U8)(gl_poly_buf[gl_poly_start].flags | 0x20 | POLY_PENDOWN)
   );
   poly_closed = 1;  //3
   return;
}

void gl_line(double x1,double y1, double x2,double y2, U8 pen_down, U8 width);

void gl_render_polygon(U8 fill)
{
   double x,y;
   double x0,y0;
   int i;
...
//printf("  %d: %f,%f .. %f,%f flags:%02X\n", i, x,y, gl_poly_buf[i].x,gl_poly_buf[i].y, gl_poly_buf[i].flags);
      if ((gl_poly_buf[i].flags & POLY_PENDOWN) || fill)   // draw line from last point to this one
         ...
         gl_line(
            ...
            (U8)(gl_poly_buf[i].flags & POLY_PENDOWN),
            gl_pw[gl_pen]
         );
         }
...
void gl_line(double x1,double y1, double x2,double y2, U8 pen_down, U8 width)
{
   ...
   if (gl_poly_number)    // in polygon definition mode - record the vertices in the polygon buffer
      {
      if (1);
      else if ((gl_poly_buf[gl_poly_index-1].x != x1) ||
               (gl_poly_buf[gl_poly_index-1].y != y1))
         {
         gl_add_poly(x1,y1, (U8)(gl_poly_number | 0x10)); // pen up move to start point
         }

      if (poly_closed) pen_down &= (~POLY_PENDOWN);
      else if (pen_down) pen_down = POLY_PENDOWN;

      gl_add_poly(x2,y2, (U8)(gl_poly_number | pen_down));
   ...
   temp_pattern_count = (U8)pattern_count;
   temp_color = color;
...
void gl_box(double x1,double y1, double x2, double y2, U8 fill)
{
   int ix1,iy1;
   int ix2,iy2;
   U8 temp_pattern;
...
void gl_wedge(double x,double y,  double r, double a1,double a2, U8 wedge)
{
   // tttt
   double x1,y1;
   double x2,y2;
...
//  color = 0;
   ...
   gl_rotate_chars = 0;
   gl_rotate_coords = (U8)user_rotate;
...
void draw_user_char(double x,double y, int font, int set_xy, int c)
{
   ...
   if (gl_rotate_coords == 3)
      ...
      if (gl_dvx == 3)
         ...
         if (set_xy);
         else if (set_xy == 0) y += gl_textx*1.000;
         }
      else if (gl_dvx == 2)
         {
...
// printf("cp: xy:%f   ofs:%f,%f  textx:%f,%f  space:%f,%f\n", gl_x,gl_y,  gl_newx,gl_newy, gl_textx,gl_texty, gl_spacex,gl_spacey);
      if ((gl_arg1[0] == 0) && (gl_arg2[0] == 0))   // no arguments - do CRLF
         ...
         gl_line(oldx, oldy,  gl_x, gl_y, gl_pendown, gl_pw[gl_pen]);    // (Needed in order to render HP 3562A with CP opcodes used to draw radical sign)
         }
      }
...
next_dl:
      ...
      {
      if (gl_newx < 4)  gl_dvx = (U8) gl_newx;
      else             gl_dvx = 0;
...
ip_window:
      ...
      if (gl_pair == 0)
         ...
         if (gl_ip_xmax == gl_ip_xmin) gl_ip_xmax += 1.0;

         if (gl_ip_ymax == gl_ip_ymin) gl_ip_ymax += 1.0;
      ...
      if (gl_pair == 0)
         ...
         if (gl_arg1[0])   // line type code
            ...
            if (gl_newx < MAX_LINE_TYPES) gl_lt = (U8) gl_newx;
            else line_pattern = 0;
            }
...
//printf("poly exit %d: glxy:%f,%f\n", gl_poly_number, gl_poly_x,gl_poly_y);
      ...
      if ((gl_arg2[0]) && (gl_newy >= 0.0) && (gl_newy < PEN_COUNT))   // set specified pen
         {
         gl_pw[(int) gl_newy] = (U8) gl_newx;
         }
      else    // set all pens
         {
         for (sx=0; sx<PEN_COUNT; sx++) gl_pw[sx] = (U8) gl_newx;
         }
...
fill_rect:
      ...
      {
      restart_line_pattern();
      gl_rotate_coords = (U8)((((int)gl_newx) % 360) / 90);
      gl_rotate_coords += (U8)user_rotate;
      gl_rotate_coords %= 4;
...
next_uc_arg:
      ...
      if (gl_pair == 0)   // first argument to the command is line type number
         ...
         gl_user_pattern[gl_user_linetype][gl_pair++] = (U8) gl_newx;
         gl_user_pattern[gl_user_linetype][gl_pair++] = (U8) gl_newy;
...
void do_gl_scale()   // ssss
{
   ...
   if (gl_sc_xmax == gl_sc_xmin) gl_sc_xmax += 1.0;

   if (gl_sc_ymax == gl_sc_ymin) gl_sc_ymax += 1.0;
...
void parse_hpgl(U8 data)
{
   ...
   if (gl_state == 0)   // first call - initialize parser
      {
      vchar_init();
      vchar_set_fontsize((U8) VX, (U8)VY);
      vchar_set_spacing(0,0);
