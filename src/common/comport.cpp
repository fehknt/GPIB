...
C8 *COMPORT::error_text(S32 last_error)
{
...
S32 COMPORT::connect(C8  *port,                   // Returns zero on success, else an error code that can be passed to error_text() 
                    S32  rate,
                    bool control_DTR,    // Resets certain devices such as Arduinos if enabled
                    bool DTR_state)    
{
   if (hSerial != INVALID_HANDLE_VALUE)
      {
...
S32 COMPORT::change_rate(S32 rate_BPS)
{
   if (hSerial == INVALID_HANDLE_VALUE)
      {
      return 0;
                   ...
                   ...)
{
   ...
   return write((U8 *) buffer, (S32)strlen(buffer));
}
...
S32 COMPORT::write(const C8 *data, 
                  S32      *written) 
{
...
S32 COMPORT::write(const U8 *data, 
                  S32       len, 
                  S32      *written) 
{
   if (hSerial == INVALID_HANDLE_VALUE)
...
S32 COMPORT::flush(S32 flush_timeout_ms)
{
   if (hSerial == INVALID_HANDLE_VALUE)
      {
      return 0;
