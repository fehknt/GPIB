...
//
// If NI GPIB support is desired, define USE_NI_GPIB and link with gpib-32.lib
// For now, we stub these out to allow linking if USE_NI_GPIB is NOT defined.
//
#ifndef USE_NI_GPIB
extern "C" {
int ibsta = 0;
int iberr = 0;
long ibcntl = 0; // Use long to match ni488.h
int __stdcall ibfindA(const char* name) { return -1; }
int __stdcall ibclr(int ud) { return -1; }
int __stdcall ibconfig(int ud, int option, int value) { return -1; }
int __stdcall ibdev(int board, int pad, int sad, int tmo, int eot, int eos) { return -1; }
int __stdcall ibloc(int ud) { return -1; }
int __stdcall ibonl(int ud, int v) { return -1; }
int __stdcall ibpad(int ud, int v) { return -1; }
int __stdcall ibrd(int ud, void* buf, long cnt) { return -1; }
int __stdcall ibrsc(int ud, int v) { return -1; }
int __stdcall ibrsp(int ud, char* spr) { return -1; }
int __stdcall ibsad(int ud, int v) { return -1; }
int __stdcall ibtmo(int ud, int v) { return -1; }
int __stdcall ibwait(int ud, int mask) { return -1; }
int __stdcall ibwrt(int ud, PVOID buf, long cnt) { return -1; }
}
#endif
...
#define VI_ATTR_TMO_VALUE 0x3FFF0002UL
...
typedef ViStatus(__stdcall *P_viOpen)(ViSession sesn, ViRsrc name,
                                      ViAccessMode mode, ViUInt32 timeout,
                                      ViSession *vi);
typedef ViStatus(__stdcall *P_viClose)(ViObject vi);
typedef ViStatus(__stdcall *P_viWrite)(ViSession vi, ViBuf buf, ViUInt32 cnt,
                                       ViPUInt32 retCnt);
typedef ViStatus(__stdcall *P_viRead)(ViSession vi, ViPBuf buf, ViUInt32 cnt,
                                      ViPUInt32 retCnt);
typedef ViStatus(__stdcall *P_viReadSTB)(ViSession vi, ViPUInt16 status);
typedef ViStatus(__stdcall *P_viSetAttribute)(ViObject vi, ViAttr attrName,
                                              ViAttrState attrValue);
typedef ViStatus(__stdcall *P_viStatusDesc)(ViObject vi, ViStatus status,
                                            ViChar desc[]);
typedef ViStatus(__stdcall *P_viClear)(ViSession vi);
typedef ViStatus(__stdcall *P_viFindRsrc)(ViSession sesn, ViString expr,
                                          ViPUInt32 vi, ViPUInt32 retCnt,
                                          ViChar desc[]);
...
//
// Automatically generate tempfile pathname, optionally open the file, and
// ensure file is closed/deleted when it goes out of scope
//
// Command-line utilities should use TEMPFNs or TEMPFILEs instead of calling
// GetTempFileName() directly, to ensure they're well-behaved in a (possibly
// concurrent) batch job.  Just declare a TEMPFILE with an optional suffix param
// for the generated filename, and access the filename with .name.
//
// To take advantage of automatic cleanup, don't call exit() while TEMPFNs are
// on the stack!
//
...
struct TEMPFN {
  C8 path_buffer[MAX_PATH - 32];        // leave some room for 8.3 tempname and
                                        // user-supplied suffix if any
  C8 original_temp_name[MAX_PATH - 32]; // e.g., c:\temp\TF43AE.tmp
  C8 name[MAX_PATH]; // e.g., c:\temp\TF43AE.tmp.wav (if .wav was passed as
                     // suffix)
  bool keep;
  bool show;
  bool active;
  ...
  TEMPFN(C8 *suffix = NULL, bool keep_files = FALSE) {
    ...
    if (!GetTempPath(sizeof(path_buffer) - 1, path_buffer)) {
      message_printf(TF_ERROR, "TEMPFN: GetTempPath() failed, code 0x%X",
                     GetLastError());
      active = FALSE;
      return;
    }
    ...
    if (!GetTempFileName(path_buffer, "TF", 0, original_temp_name)) {
      message_printf(TF_ERROR, "TEMPFN: GetTempFileName() failed, code 0x%X",
                     GetLastError());
      active = FALSE;
      return;
    }
    ...
    if (suffix == NULL) {
      ...
      //
      // A suffix was provided: the caller will create *another* tempfile whose
      // name will begin with the guaranteed-unique name of the first one.  This
      // file also must be cleaned up
      //

      strncat(name, suffix, 15);
    }

    message_printf(TF_VERBOSE, "TEMPFN: original_temp_name: %s",
                   original_temp_name);
  ...
  virtual ~TEMPFN() {
    //
    // Ensure that both the original tempfile and the user-extended version (if
    // any) are cleaned up at end of scope
    //
  ...
  virtual bool status(void) { return active; }
};

struct TEMPFILE : public TEMPFN {
  FILE *file;

  TEMPFILE(C8 *suffix = NULL, C8 *file_operation = NULL,
           bool keep_files = FALSE)
      : TEMPFN(suffix, keep_files)

  {
    ...
    if (file == NULL) {
      message_printf(TF_ERROR, "TEMPFILE: Couldn't open %s for %s", name,
                     (tolower((U8)file_operation[0]) == 'r') ? "reading"
                                                             : "writing");
...
struct APPDIRS {
  C8 EXE[MAX_PATH];  // Directory containing .exe corresponding to current
                     // process
  C8 DOCS[MAX_PATH]; // Directory where user documents are to be loaded/saved by
                     // default
  ...
  C8 VCDATA[MAX_PATH]; // Directory for vendor .INI files and data common to all
                       // users
  C8 LOCDATA[MAX_PATH]; // Directory for user-specific application data (not
                        // good for .INI files unless they're genuinely
                        // user-specific)
  C8 COMDATA[MAX_PATH]; // Directory for application .INI files common to all
                        // users
  ...
  bool initialized(void) { return (EXE[0] != 0); }

  void
  init(C8 *vendor_name =
           NULL, // Vendor name that refers to VLDATA and VCDATA subdirectories
                 // beneath %localappdata% and %programdata%
       C8 *app_name = NULL) // App name used to reference LOCDATA and COMDATA
                            // subdirectories underneath VLDATA and VCDATA
  {
    //
    ...
    //
    // See http://msdn.microsoft.com/en-us/library/ms995853.aspx for data and
    // settings management guidelines.  Vista replaced CSIDL identifiers with
    // GUIDs but the legacy functions are still used for backwards compatibility
    //
    // Furthermore, SHGetSpecialFolderPath() was deprecated in favor of
    // SHGetFolderPath(), but the latter has various Windows
    // header/macro-definition dependencies, and there doesn't seem to be any
    // real reason to switch to it
    //
    ...
    //
    //    CSIDL_LOCAL_APPDATA     c:\documents and settings\johnm\local settings
    //    (hidden)\application data CSIDL_APPDATA           c:\documents and
    //    settings\johnm\application data CSIDL_COMMON_APPDATA    c:\documents
    //    and settings\all users\application data (hidden)
    //
    // On Windows 7:
    //
    ...
    //
    //    CSIDL_LOCAL_APPDATA     c:\users\johnm\appdata\local (%localappdata%
    //    in both OS and Inno Setup) CSIDL_APPDATA
    //    c:\users\johnm\appdata\roaming  (%appdata%) CSIDL_COMMON_APPDATA
    //    c:\ProgramData                  (%programdata%, or %commonappdata% in
    //    Inno Setup)
    //
    ...
    SHGetSpecialFolderPath(HWND_DESKTOP, DESKTOP, CSIDL_DESKTOPDIRECTORY,
                           FALSE);

    SHGetSpecialFolderPath(HWND_DESKTOP, mydocs, CSIDL_PERSONAL, FALSE);

    SHGetSpecialFolderPath(HWND_DESKTOP, local_appdata, CSIDL_LOCAL_APPDATA,
                           FALSE);

    SHGetSpecialFolderPath(HWND_DESKTOP, common_appdata, CSIDL_COMMON_APPDATA,
                           FALSE);
    ...
    // e.g., c:\documents and settings\johnm\local settings\application
    // data\vendor_name\app_name\
      //
    // These directories may not have been created at install time if a
    // different user (e.g, an administrator) ran the installer
    //
    ...
    // to the current working directory.  This also permits use of the "Start
    // In" field in a desktop shortcut.
    //
    // If the CWD is the Windows desktop, contains "Program Files", or could not
    // be determined, the default docs directory is set to the current user's My
    // Documents folder
    //

    strcpy(DOCS, ICW);

    if ((!strlen(DOCS)) || (!_stricmp(DOCS, DESKTOP)) ||
...
bool prologix_3012c = FALSE;
bool prologix_V4 = FALSE;
bool prologix_4040 = FALSE;
bool prologix_6102 = FALSE;
bool prologix_auto_read_enabled = TRUE;
bool prologix_was_auto_read_enabled = TRUE;
bool prologix_listen_only_mode = FALSE;
...
bool INI_is_NI488 = FALSE;
bool INI_is_TCP = FALSE;
bool INI_is_VISA = FALSE;
bool INI_is_Prologix = FALSE;
S32 INI_write_delay_ms = 0;
S32 INI_reset_to_local = 0;
bool INI_restore_auto_read = FALSE;
bool INI_enable_LON = TRUE;
bool INI_force_auto_read = TRUE;
HANDLE hCOMSem = NULL;

COMBLOCK *serial_device = NULL;

static HMODULE hVISA = NULL;
static P_viOpenDefaultRM p_viOpenDefaultRM = NULL;
static P_viOpen p_viOpen = NULL;
static P_viClose p_viClose = NULL;
static P_viWrite p_viWrite = NULL;
static P_viRead p_viRead = NULL;
static P_viReadSTB p_viReadSTB = NULL;
...
static bool load_visa(void) {
  if (hVISA != NULL)
    return TRUE;

  hVISA = LoadLibrary("visa32.dll");
  if (hVISA == NULL)
    return FALSE;

  p_viOpenDefaultRM =
      (P_viOpenDefaultRM)GetProcAddress(hVISA, "viOpenDefaultRM");
  p_viOpen = (P_viOpen)GetProcAddress(hVISA, "viOpen");
  p_viClose = (P_viClose)GetProcAddress(hVISA, "viClose");
  p_viWrite = (P_viWrite)GetProcAddress(hVISA, "viWrite");
  p_viRead = (P_viRead)GetProcAddress(hVISA, "viRead");
  p_viReadSTB = (P_viReadSTB)GetProcAddress(hVISA, "viReadSTB");
  ...
  p_viGpibControlREN =
      (P_viGpibControlREN)GetProcAddress(hVISA, "viGpibControlREN");

  if (!p_viOpenDefaultRM || !p_viOpen || !p_viClose || !p_viWrite ||
      !p_viRead || !p_viFindRsrc || !p_viFindNext)
    return FALSE;

  return TRUE;
}

// Shared libraries can't easily export variables, so we use a function
GPIBDEF void WINAPI GPIB_set_serial_device(COMBLOCK *device) {
  ...
  prologix_4040 = 0;
  prologix_6102 = 0;
  prologix_auto_read_enabled = 1;
  prologix_was_auto_read_enabled = 1;
  prologix_listen_only_mode = 0;
...
S32 WINAPI GPIB_online(void) {
  ...
  HANDLE hPrologixSem = CreateSemaphore(NULL, 0, 1, "Prologix");
  ...
  hCOMSem = CreateSemaphore(NULL, 0, 1, INI_setup_string);

  if ((hCOMSem != NULL) && (GetLastError() == ERROR_ALREADY_EXISTS)) {
    CloseHandle(hCOMSem);
    hCOMSem = NULL;

    GpibError("Port in use -- please exit from all other GPIB Toolkit "
              "applications first\n\n");
    return 0;
  }
  ...
  if (!INI_is_NI488 && !INI_is_VISA) {
    ...
    S32 BoardIndex = atoi(
        &INI_setup_string[4]); // Get interface index from GPIB0...GPIBnn string

    UINT err = SetErrorMode(SEM_NOOPENFILEERRORBOX | SEM_FAILCRITICALERRORS);
    GPIB_ID = ibfind(INI_setup_string); // avoid errors due to partial uninstall
                                        // of old NI drivers
    SetErrorMode(err);
    ...
    if (GPIB_ID == -1) {
      ...
      GpibError(
          "National Instruments board (%s) not found\n\nAre you "
          "using a Prologix adapter?  If so, you must run the\n"
          "GPIB Configurator application first.  Select \"Update "
          "CONNECT.INI\"\nafter selecting your adapter's COM port or IP "
          "address.\n\n"
          //                   "- Are you running Windows Vista or later
          //                   versions?  If so, you may\n" "need to mark all
          //                   GPIB Toolkit applications, including
          //                   Prologix.exe,\n" "to run in Windows XP
          //                   compatibility mode with full Administrator\n"
          //                   "rights. Then, run PROLOGIX.EXE and select
          //                   \"Update CONNECT.INI\" again.\n\n"
          "Otherwise, you must either configure your NI488.2-compatible\nGPIB "
          "adapter as %s, "
          "or use the GPIB Configurator to select the\ncorrect "
          "board.\n\n\Users of Agilent "
          "or Keysight GPIB adapters must enable NI488.2\ncompatibility.  "
          "Otherwise, the "
          "adapter will be unavailable for\nselection in the GPIB "
          "Configurator.\n\n",
          INI_setup_string, INI_setup_string);
    ...
    //         {                                 // (Enabling this breaks NI
    //         compatibility...) Device = ibdev(                     /* Create a
    //         unit descriptor handle (necessary on ICS but not NI)   */
    //               BoardIndex,                   /* Board Index (GPIB0 = 0,
    //               GPIB1 = 1, ...) */ 0,                            /* Device
    //               primary address                  */ SecondaryAddress, /*
    //               Device secondary address                */
    //               GPT(timeout_period_msecs),    /* Timeout setting (T10s = 10
    //               seconds)     */ 1,                            /* Assert EOI
    //               line at end of write         */ 0); /* EOS termination mode
    //               */
    //         }
    //      else                                   // (Device is the target
    //      instrument)
    {
...
bool WINAPI GPIB_offline(S32 reset_to_local) {
  if (!online) {
    return TRUE;
  }
  ...
  if (!INI_is_NI488 && !INI_is_VISA) {
    ...
    // its original state before transmitting ++loc at shutdown time (old
    // behavior), 0=always leave auto-read disabled at shutdown time.  Default
    // is 0
    //

    if (serial_device != NULL) {
      if (INI_is_Prologix) {
        if ((!prologix_3012c) && (!INI_is_TCP)) {
          prologix_auto_read_enabled = TRUE;
        } else {
          if (device_address != -1) {
            if (prologix_auto_read_enabled &&
                ((!prologix_was_auto_read_enabled) ||
                 (INI_restore_auto_read == 0))) {
              GPIB_write("++auto 0");
              prologix_auto_read_enabled = FALSE;
            } else if ((!prologix_auto_read_enabled) &&
                       prologix_was_auto_read_enabled &&
                       INI_restore_auto_read) {
              GPIB_write("++auto 1");
              prologix_auto_read_enabled = TRUE;
            }
          }

          if (INI_reset_to_local && reset_to_local) {
            //
            // HP 5370 chatters if auto-read enabled at shutdown time when using
            // rates >1pps HP 53131 will not reset to local with GPIB-USB if
            // auto-read is *not* enabled at shutdown time
            //

            if (INI_reset_to_local == 53131)
              GPIB_write("++auto 1");
            GPIB_write("++loc");
          }
        }

        GPIB_flush_receive_buffers(); // Swallow any unread data before
                                      // disconnecting TODO
      }
    ...
    if (Device != -1) {
      if (reset_to_local) {
        ibloc(Device); // Reset to local (8566B rev 3022 doesn't do this
                       // automatically)
      }

      ibonl(Device, 0); /* Take the device offline                 */

      //         if (ibsta & ERR)             // These report spurious timeout
      //         errors on ICS USB-488 if a previous ibrd() timed out,
...
static S32 extract_visa_addr(const C8 *desc) {
  ...
  if (gpib_part && sscanf(gpib_part, "GPIB_%d_", &addr) == 1)
    return addr;
  if (sscanf(desc, "GPIB%*d::%d::INSTR", &addr) == 1)
    return addr;
  if (sscanf(desc, "GPIB::%d::INSTR", &addr) == 1)
    return addr;
  return -1;
}

bool WINAPI GPIB_connect(S32 dev_address, GPIBERR handler, bool clear,
                         S32 timeout_msecs, S32 board_address,
                         bool release_system_control, S32 max_read_buffer_len,
                         S32 force_auto_read) {
  ...
  // The only exceptions are the transmission of ++mode 0 and ++lon to a
  // Prologix adapter
  //
  ...
  Sleep(25); // Needed to avoid ENOL errors on HP 3562A and syntax errors on HP
             // 3561A, at least with NI
  ...
  if (!INI_is_NI488 && !INI_is_VISA) {
    if (prologix_listen_only_mode) { // No queries, no buffer flushing, no
                                     // address alteration... although we do
      prologix_3012c = 0; // send ++lon to set up listen-only mode, unless it's
                          // disabled in connect.ini
      prologix_V4 = 0;
      prologix_4040 = 0;
      prologix_6102 = 0;
      ...
      if (INI_enable_LON) {
        GPIB_write("++lon 1"); // Not supported for FW < 6.102, will need to
                               // disable in connect.ini if this causes problems
      }
      ...
      if (INI_is_TCP) {
        GPIB_write(
            "++eot_enable 0"); // some GPIB-LAN adapters seem to have this
                               // turned on by default, which is a showstopper
      }

      C8 *ver = GPIB_query("++ver", TRUE);
      C8 *v = ver ? strstr(ver, "ersion ") : NULL;

      if (v != NULL) {
        sscanf(v, "ersion %d.%d", &prologix_major_version,
      ...
      S32 n = (prologix_major_version * 1000) + prologix_minor_version;

      if (n >= 4000) {
        prologix_3012c = prologix_V4 = TRUE;
        ...
        if ((n > 3012) || ((n == 3012) && (prologix_letter_version >= 'c'))) {
          prologix_3012c = TRUE;
        }
      }
      ...
      if (device_address != -1) {
        //
        // We should issue ++mode 1 here, but this causes the 8568A to flake out
        // occasionally if the Prologix adapter is already in controller mode
        // with auto read off
        //
        // The ++auto commands don't seem to hurt anything in device mode, even
        // though they aren't supported.  If we get "Unrecognized command",
        // we'll go ahead and send ++mode 1 and then retry the query...
        //

        if (prologix_3012c || (INI_is_Prologix && INI_is_TCP)) {
          C8 *a = GPIB_query("++auto", TRUE);

          if (a && a[0] == 'U') {
            GPIB_write("++mode 1");
            a = GPIB_query("++auto", TRUE);
          }
      ...
      GPIB_write("++eos 0"); // we depend on CR/LF transmission in both modes

      C8 buffer[256];

      if (device_address != -1) {
        GPIB_write("++mode 1"); // if we're addressing a device, we need
                                // Controller mode
        ...
        GPIB_write(
            "++mode 0"); // if no device address specified, use Device mode

        if (board_address != -1) {
          if (INI_enable_LON && prologix_6102) {
            GPIB_write("++lon 0");
          }
      ...
      GPIB_flush_receive_buffers(); // Swallow any data left over from previous
                                    // connection
    }
  } else if (INI_is_NI488) {
    ibsad(GPIB_ID, 0);
    ...
    if (board_address != -1) {
      ibpad(GPIB_ID, board_address); // ibconfig version of this call is buggy
                                     // on ICS 488-USB
    ...
    if (release_system_control) {
      ibrsc(GPIB_ID, 0); // Don't release by default; can't turn on 49x
                         // graticule if released
    } else {
      //
      // If we're not already CIC, assert it by sending an interface-clear
      // message
      //
      // This is necessary for NI GPIB-USB-A with 1.1 drivers, as well as ICS
      // 488-USB boards, but to avoid breaking something else we won't do it
      // unless we have to
      //
      // TODO: Causes async_read() to fail with ADR error when waiting for
      // device-initiated plots on NI PCI-GPIB, so we'll leave it disabled for
      // now
      //
    ...
    GPIB_set_EOS_mode(
        -1, TRUE,
        FALSE); // 8566A/8568A apps will need to set EOT mode to FALSE...
    ...
    if (device_address != -1) {
      if (extract_visa_addr(INI_setup_string) != device_address) {
        ...
        if (p_viFindRsrc(defaultRM, "?*::INSTR", &findList, &retCnt, desc) ==
            0) {
          do {
    ...
    ViStatus status =
        p_viOpen(defaultRM, target_resource, VI_NULL, VI_NULL, &instr);

    if (status < 0) {
      // Fallback: try to open a matching instrument by address, or the first
      // available if address is -1
      ViUInt32 findList;
      ViUInt32 retCnt;
      ViChar desc[256];
      if (p_viFindRsrc(defaultRM, "?*::INSTR", &findList, &retCnt, desc) >= 0) {
        do {
          if (device_address == -1 ||
              extract_visa_addr(desc) == device_address) {
            strcpy(target_resource, desc);
            status =
                p_viOpen(defaultRM, target_resource, VI_NULL, VI_NULL, &instr);
            if (status >= 0)
...
bool WINAPI GPIB_connect_ex(C8 *dev_address, GPIBERR handler, bool clear,
                            S32 timeout_msecs, S32 board_address,
                            bool release_system_control,
                            S32 max_read_buffer_len, S32 force_auto_read) {
  //
  // If dev_address contains a colon, treat the portion before it
  // as an interface_settings line for Prologix (COMx:addr) or NI (GPIBx:addr)
  // boards
  //
  ...
  if ((strstr(dup, "::") != NULL) || (!_strnicmp(dup, "USB", 3))) {
    ...
    INI_connection_type = GC_NI488; // Treat as NI488 for now to avoid breaking
                                    // apps checking for serial/lan
    INI_is_Prologix = 0;
    INI_is_TCP = 0;
    INI_is_NI488 = 0;
    INI_is_VISA = 1;
  ...
  return GPIB_connect(dev_addr, handler, clear, timeout_msecs, board_address,
                      release_system_control, max_read_buffer_len, -1);
}

bool WINAPI GPIB_disconnect(bool reset_to_local) {
  bool result = GPIB_offline(reset_to_local);
...
S32 WINAPI GPIB_flush_receive_buffers(void) {
  ...
  while (1) {
    ...
    if ((r == SR_ERROR) || (r == SR_DISCONNECTED) ||
        ((r == SR_TIMEOUT) && (b == 0))) {
      break;
    }
  }
...
GPIB_CTYPE WINAPI GPIB_connection_type(void) {
  ...
  //
  // If GPIB_connect_ex() has been called, don't load or report connect.ini
  // settings
  //
  ...
  _snprintf(INI_user_pathname, sizeof(INI_user_pathname) - 1, "%sconnect.ini",
            global_dirs.LOCDATA);
  ...
  if (ser != NULL) {
    ...
    _snprintf(default_INI_pathname, sizeof(default_INI_pathname) - 1,
              "%sdefault_connect.ini", global_dirs.EXE);

    CopyFile(default_INI_pathname, INI_user_pathname, FALSE);
  }
  ...
  _snprintf(INI_working_pathname, sizeof(INI_working_pathname) - 1,
            "%sconnect.ini", global_dirs.ICW);
  ...
  //
  // connect.ini found -- if it has a valid interface_settings line other than
  // one beginning with "GPIB", we'll be able to construct a COMBLOCK provider
  //
  ...
  while (1) {
    ...
    C8 *result = fgets(linbuf, sizeof(linbuf) - 1, ser);
    ...
    *value = 0;
    ...
    if (!_stricmp(lexeme, "is_Prologix"))
      INI_is_Prologix = atoi(value) != 0;
    else if (!_stricmp(lexeme, "reset_to_local"))
      INI_reset_to_local = atoi(value) != 0;
    else if (!_stricmp(lexeme, "restore_auto_read"))
      INI_restore_auto_read = atoi(value) != 0;
    else if (!_stricmp(lexeme, "force_auto_read"))
      INI_force_auto_read = atoi(value) != 0;
    else if (!_stricmp(lexeme, "enable_LON"))
      INI_enable_LON = atoi(value) != 0;
    else if (!_stricmp(lexeme, "write_delay_ms"))
      INI_write_delay_ms = atoi(value);
    else if (!_stricmp(lexeme, "interface_settings"))
      strcpy(INI_setup_string, value);
    else {
  ...
  if (prologix_listen_only_mode) // (a previous call to GPIB_connect requested
                                 // listen-only support)
  {
  ...
  } else if ((strstr(INI_setup_string, "::") != NULL) ||
             (!_strnicmp(INI_setup_string, "USB", 3))) {
    INI_is_VISA = TRUE;
    INI_connection_type = GC_NI488; // Treat as NI488 for now to avoid breaking
                                    // apps checking for serial/lan
  } else {
    INI_connection_type =
        INI_is_Prologix ? GC_PROLOGIX_SERIAL : GC_OTHER_SERIAL;
  }
...
S32 WINAPI GPIB_set_serial_read_dropout(S32 ms) {
  ...
  if (INI_is_Prologix &&
      (!prologix_auto_read_enabled)) // (Technically ++spoll uses this too, so
                                     // we should always do it...)
  {
    char buffer[64];
    sprintf(buffer, "++read_tmo_ms %d",
            std::min(ms, 3000)); // Prologix ignores anything > 3000
    GPIB_write(buffer);
  }
...
bool WINAPI GPIB_auto_read_mode(S32 set_mode) {
  if (!(prologix_3012c || (INI_is_Prologix && INI_is_TCP))) {
    return FALSE; // Only Prologix >3.12c supports auto-read mode
  }

  if (device_address != -1) {
    return TRUE; // ++read not supported in device mode, where data always comes
                 // in automatically
  }
...
SRPROGRESSCB WINAPI
GPIB_set_serial_read_progress_callback(SRPROGRESSCB progress_cb) {
...
S32 WINAPI GPIB_set_EOS_mode(S32 new_EOS_char,
                             bool send_EOI_at_EOT,
                             bool enable_board_configuration) {
  S32 prev = EOS_char;
  ...
  if (INI_is_NI488 || INI_is_VISA) {
    ...
    //
    // On HP 8566A/8568A, we must not assert EOI at the last character of a
    // query command (send_EOI_at_EOT must be 0).  Otherwise, the read will time
    // out (although it appears to be OK to set EOI at EOT if the query ends in
    // \r or \r\n).
    //
    // We never want to automatically assert EOI during EOS-character writes
    // (IbcEOSwrt is always 0)
    //

    if (Device != -1 && INI_is_NI488) {
      ibconfig(Device, IbcEOT,
               send_EOI_at_EOT); // (optionally assert EOS with last byte)
      ibconfig(Device, IbcEOSwrt,
               0); // (don't assert EOI when we send EOS character)
    ...
    } else if (enable_board_configuration &&
               INI_is_NI488) // (Added 2/07 for listen.exe: can turn this off if
                             // it breaks existing apps...)
    {
      ...
      p_viSetAttribute(instr, VI_ATTR_SEND_END_EN,
                       send_EOI_at_EOT ? VI_TRUE : VI_FALSE);
      if (EOS_char == -1) {
    ...
    //
    // Serial routines do their own IbcEOSchar/IbcEOSrd checks when appropriate,
    // but we do need to control outgoing-EOI-at-last-byte assertion here
    // (IbcEOT equivalent).
    //
...
S32 WINAPI GPIB_write(C8 *string,
                      S32 len, bool to_board, bool ignore_aborts) {
  ...
  buffer[len] = 0; // Always append CR/LF (which will be removed and replaced
                   // with the ++eos string by Prologix)
  strcat(buffer, "\r\n");
  len += 2;

  if (!INI_is_NI488 && !INI_is_VISA) {
    Sleep(INI_write_delay_ms / 2);
    if (!serial_device->send(buffer, len)) {
      return 0;
    }
    Sleep(INI_write_delay_ms / 2);
...
S32 WINAPI GPIB_write_BIN(void *string,
  ...
  if (!INI_is_NI488 && !INI_is_VISA) {
    ...
    ViUInt32 retCnt = 0;
    p_viWrite(instr, (ViBuf)string, len, &retCnt);
  }
...
C8 *WINAPI GPIB_read_ASC(S32 max_len,
                         bool report_timeout, bool from_board) {
  //
  ...
  if (!INI_is_NI488 && !INI_is_VISA) {
    ...
    U8 *result = serial_device->receive(
        max_len,
        from_board ? 10 : EOS_char, // (assume board-level reads such as ++spoll
                                    // are always LF-terminated)
        serial_timeout_ms, serial_dropout_ms, serial_progress_cb, &cnt,
        &reason);

    if ((reason == SR_TIMEOUT) || (reason == SR_ERROR) ||
...
C8 *WINAPI GPIB_read_BIN(S32 max_len,
  ...
  if (!INI_is_NI488 && !INI_is_VISA) {
    ...
    U8 *result = serial_device->receive(max_len, -1, serial_timeout_ms,
                                        serial_dropout_ms, serial_progress_cb,
                                        &cnt, &reason);
    ...
    if (status < 0) {
      iberr = status;
      if (report_timeout)
        GpibError("viRead_BIN Error: 0x%08X", status);
      return NULL;
    }
    if (actual_len)
      *actual_len = retCnt;
...
S32 WINAPI GPIB_status(bool from_board) {
  if (!INI_is_NI488 && !INI_is_VISA) {
    return 0;
  }
...
S32 WINAPI GPIB_puts(C8 *string, bool ignore_aborts) {
  S32 result = GPIB_write(string, -1, FALSE, ignore_aborts);
  return result;
}
...
void __cdecl GPIB_printf(C8 *fmt, ...) {
  ...
  _vsnprintf(work_string, sizeof(work_string) - 1, fmt, ap);
...
#define APPEND (&errbuf[strlen(errbuf)])
  ...
  if (!INI_is_NI488 && !INI_is_VISA) {
    ...
    if (iberr == EDVR)
      sprintf(APPEND, " EDVR <DOS Error>\n");
    if (iberr == ECIC)
      sprintf(APPEND, " ECIC <Not Controller-In-Charge>\n");
    if (iberr == ENOL)
      sprintf(APPEND, " ENOL <No Listener>\n");
    if (iberr == EADR)
      sprintf(APPEND, " EADR <Address error>\n");
    if (iberr == EARG)
      sprintf(APPEND, " EARG <Invalid argument>\n");
    if (iberr == ESAC)
      sprintf(APPEND, " ESAC <Not System Controller>\n");
    if (iberr == EABO)
      sprintf(APPEND, " EABO <Operation aborted>\n");
    if (iberr == EHDL)
      sprintf(APPEND, " EHDL <Invalid input handle>\n");
    if (iberr == ENEB)
      sprintf(APPEND, " ENEB <No GPIB board>\n");
    if (iberr == EOIP)
      sprintf(APPEND, " EOIP <Async I/O in progress>\n");
    if (iberr == ECAP)
      sprintf(APPEND, " ECAP <No capability>\n");
    if (iberr == EFSO)
      sprintf(APPEND, " EFSO <File system error>\n");
    if (iberr == EBUS)
      sprintf(APPEND, " EBUS <Command error>\n");
    if (iberr == ESTB)
      sprintf(APPEND, " ESTB <Status byte lost>\n");
    if (iberr == ESRQ)
      sprintf(APPEND, " ESRQ <SRQ stuck on>\n");
    if (iberr == ETAB)
      sprintf(APPEND, " ETAB <Table Overflow>\n");
  ...
  if (err_handler != NULL) {
    ...
    MessageBox(NULL, errbuf, "GPIBLIB Fatal Error", MB_OK);
    exit(1);
  }
