...
namespace SPARAM
{
   MA::MA(DOUBLE m, DOUBLE a) : mag(m), deg(a) {} 
   ...
   DB::DB(DOUBLE d, DOUBLE a) : dB(d), deg(a) {} 

   DB::DB(MA MA)
      {
      dB  = 20.0 * log10(std::max(1E-15, MA.mag));
      deg =              MA.deg;
      }
   ...
   DB::DB(RI RI)
      ...
      dB  = 20.0 * log10(std::max(1E-15, sqrt(i*i + q*q)));
   ...
   RI::RI(DOUBLE r, DOUBLE i) : COMPLEX_DOUBLE(r,i) {}

   RI::RI(COMPLEX_DOUBLE c)   : COMPLEX_DOUBLE(c)   {}
   ...
   CZ::CZ(DOUBLE r, DOUBLE x) : R(r),  jX(x) {} 
...
void SPARAMS::message_sink(SPARAM::MSGLVL level, C8 *text)
   {
   ::printf("%s\n",text);
   }

void SPARAMS::message_printf(SPARAM::MSGLVL level,
                            C8            *fmt,
                            ...)
   {
   va_list ap;
...
bool SPARAMS::alloc(S32 ports, S32 points)
   {
   if ((points == 0) || (ports == 0))
      {
      message_printf(SPARAM::MSG_ERROR,"Empty data set");
      return FALSE;
      }
   ...
   if (freq_Hz == NULL)
      {
      message_printf(SPARAM::MSG_ERROR,"Out of memory");
      return FALSE;
      }
...
U8 *SPARAMS::serialize(S32 *output_bytes)
   {
   S32 n_data_bytes = 0;
...
S32 SPARAMS::deserialize(FILE *in)
   {
   #pragma pack(push,1)
   struct _HDR
      {
   ...
   if (fread(&HDR.ID, sizeof(HDR.ID), 1, in) != 1)
      {
      message_printf(SPARAM::MSG_ERROR,"Couldn't read from SNPB file");
      return -1;
      }
   ...
   if (HDR.ID != SPARAM::BIN_ID)
      ...
      message_printf(SPARAM::MSG_VERBOSE, "Unrecognized block ID");
      return 0;
      }
   ...
   if (fread(block, HDR.n_data_bytes, 1, in) != 1)
      {
      message_printf(SPARAM::MSG_ERROR,"Couldn't read data from SNPB file");
      return -1;
      }
...
S32 SPARAMS::deserialize(U8 *block)
   {
   ...
   if (read_version != SPARAM::BIN_VERSION)
      {
      message_printf(SPARAM::MSG_ERROR,"Binary version 0x.08X not supported by version 0x.08X parser", read_version, SPARAM::BIN_VERSION);
      return -1;
      }
   ...
   if (alloc(read_n_ports, read_n_points))
      ...
      if (bytes_read != expect_n_data_bytes)
         {
         message_printf(SPARAM::MSG_ERROR,"Missing or corrupt binary SNP data (%d bytes expected, %d read)",
            expect_n_data_bytes,
            bytes_read);
...
// --------------------------------------------------------------------------------------------------
// Remove Touchstone-incompatible characters from string
// --------------------------------------------------------------------------------------------------

C8 *SPARAMS::sanitize(const C8 *input)
   {
...
int SPARAMS::search_double_array(const void *keyval, const void *datum)
   {
...
S32 SPARAMS::nearest_freq_Hz(DOUBLE Hz, DOUBLE *alpha)
   {
   if (Hz <= freq_Hz[0])         
      {
...
bool SPARAMS::point_valid(S32 pt, S32 param)
   {
...
void SPARAMS::set_RI(S32 pt, S32 param, COMPLEX_DOUBLE val)
   {
   const S32 B[4] = { 1, 2, 1, 2 };
   const S32 A[4] = { 1, 1, 2, 2 };
...
void SPARAMS::set_RI(S32 pt, S32 b, S32 a, COMPLEX_DOUBLE val)
   {
   RI    [b][a][pt] = val;
   valid [b][a][pt] = SNPTYPE::RI;
   }
...
SPARAM::RI SPARAMS::get_RI(S32 pt, S32 param)
   {
   const S32 b[4] = { 1, 2, 1, 2 };
   const S32 a[4] = { 1, 1, 2, 2 };
...
SPARAM::RI SPARAMS::get_RI(S32 pt, S32 b, S32 a)
   {
...
SPARAM::MA SPARAMS::get_MA(S32 pt, S32 param)
   {
   const S32 b[4] = { 1, 2, 1, 2 };
   const S32 a[4] = { 1, 1, 2, 2 };
...
SPARAM::MA SPARAMS::get_MA(S32 pt, S32 b, S32 a)
   {
   if (valid[b][a][pt] & SNPTYPE::MA)
      {
      return MA[b][a][pt];                         
...
SPARAM::DB SPARAMS::get_DB(S32 pt, S32 param)
   {
   const S32 b[4] = { 1, 2, 1, 2 };
   const S32 a[4] = { 1, 1, 2, 2 };
...
SPARAM::DB SPARAMS::get_DB(S32 pt, S32 b, S32 a)
   {
   if (valid[b][a][pt] & SNPTYPE::DB)
      {
      return DB[b][a][pt];                         
...
SPARAM::CZ SPARAMS::get_CZ(S32 pt, S32 param)
   {
   const S32 b[4] = { 1, 2, 1, 2 };
   const S32 a[4] = { 1, 1, 2, 2 };
...
SPARAM::CZ SPARAMS::get_CZ(S32 pt, S32 b, S32 a)
   {
...
SPARAM::RI SPARAMS::get_RI(DOUBLE Hz, S32 b, S32 a, U8 flags, bool *in_range)
   {
   if (in_range != NULL) 
      {
      *in_range = TRUE;
...
SPARAM::MA SPARAMS::get_MA(DOUBLE Hz, S32 b, S32 a, U8 flags, bool *in_range)
   {
   if (in_range != NULL) 
      {
      *in_range = TRUE;
...
SPARAM::DB SPARAMS::get_DB(DOUBLE Hz, S32 b, S32 a, U8 flags, bool *in_range)
   {
   return SPARAM::DB(get_MA(Hz, b, a, flags, in_range));
   }

SPARAM::CZ SPARAMS::get_CZ(DOUBLE Hz, S32 b, S32 a, U8 flags, bool *in_range)
   {
...
bool SPARAMS::T_check(DOUBLE *out)
   {
   for (S32 pt=0; pt < n_points; pt++)
      ...
      out[pt] = VNA_MATH::calculate_tcheck(RI[0][0][pt], RI[0][1][pt], RI[1][0][pt], RI[1][1][pt]);
      }
...
bool SPARAMS::write_SNP_file(const C8 *filename,   
                            const C8 *data_format,       
                            const C8 *freq_format,       
                            const C8 *header,            
                            const C8 *single_param_type) 
   {
   if (n_ports > 2)
      {
      message_printf(SPARAM::MSG_ERROR,">2 ports not supported");
      return FALSE;
      }
   ...
   if (out == NULL)
      {
      message_printf(SPARAM::MSG_ERROR,"Couldn't open %s", filename);
      return FALSE;
      }
   ...
   for (S32 i=0; i < n_points; i++)
      {
      fprintf(out,"%.16lf ", freq_Hz[i] / freq_fac[freq_fmt]);
...
bool SPARAMS::read_SNP_file(const C8 *filename, 
                           S32       file_ports)
   {
   //
   ...
   for (S32 pass=1; pass <= 2; pass++)
      ...
      if (pass == 2)
         {
         message_printf(SPARAM::MSG_VERBOSE, "  Points: %d\n", file_points);
      ...
      for (;;)
         ...
         if (txt[0] == '[')
            {
            message_printf(SPARAM::MSG_ERROR,"Touchstone 2.0 and later files not supported");
            fclose(in);
            return FALSE;
            }
         ...
         if (txt[0] == '#')
            ...
            if (file_points == 0)      // must be the first non-comment line in the file
               ...
               while (*src)
                  ...
                  if (!isspace((U8) *src))
                     {
                     message_printf(SPARAM::MSG_WARNING, "Unknown option '%s' in %s\n",src, filename);
                     }

                  src++;
                  }

               message_printf(SPARAM::MSG_VERBOSE, "\nFilename: %s\n  Header: %s\n   Scale: %lf\n   Param: %c\n    Type: 0x%.2X\n       R: %lf\n", 
                  filename, txt, file_scale, file_param, file_format, file_R);

               if (file_param != 'S')
                  {
                  message_printf(SPARAM::MSG_ERROR,"%c-parameter files not supported", file_param);
                  fclose(in);
                  return FALSE;
                  }
               }
         ...
         if (freq_Hz[pt] < max_Hz)
            {
            message_printf(SPARAM::MSG_VERBOSE, "  Notice: Truncating file to %d points due to presence of noise record\n", pt);
   ...
   message_printf(SPARAM::MSG_VERBOSE, "  Min Hz: %lf\n  Max Hz: %lf\n", min_Hz, max_Hz);
   message_printf(SPARAM::MSG_VERBOSE, "\n");
...
void SPARAMS::interp_Hz(DOUBLE  out_min_Hz,
                       DOUBLE  out_max_Hz,
                       S32     n_out_points,
                       DOUBLE *out_Hz,
                       S32    *p0,
...
void SPARAMS::preset_Hz_range(void)
   {
   assert(n_points > 1);
   assert(n_points & 1);

   DOUBLE Hz   = min_Hz;
   DOUBLE d_Hz = (max_Hz - min_Hz) / (n_points-1);    // frequency range is inclusive, intended to be used with odd point counts
...
void SPARAMS::spline_dB(S32     b,
   ...
   if (out_Hz == NULL)                          // Return interpolated linear magnitude in dB 
      ...
      for (S32 i=0; i < n_out_points; i++)      // Clamp the log10() argument since steep edges can cause ringing into the negative range
         {
         out_dB[i] = 20.0 * log10(std::max(1E-15,dest_Y[i])); 
         }
      }
   else
      {
