...
static void * app_alloc(long bytes) { return malloc(bytes); }
static void * app_zero_alloc(long bytes) { return calloc(1, bytes); }
static void app_free(void *ptr) { free(ptr); }
static void * app_realloc(void *ptr, long bytes) { return realloc(ptr, bytes); }
static FILE * app_open_file(const char *name, const char *mode) { return fopen(name, mode); }
static void app_close_file(FILE *file) { fclose(file); }
...
void read_gif_palette(FILE *file, GifPalette *cmap)
{
   ...
   for (i=0; i<cmap->length; i++) {
      ...
      g = read_byte(file);
      b = read_byte(file);
      cmap->colours[i] = app_new_rgb(r,g,b);
   }
}
   ...
   Gif *gif = new_gif();
	...
	for (S32 i=0; i < 256; i++) 
      ...
      if ((palette[i].r >= 0xf8) &&
		...
		cmap->colours[i] = app_new_rgb(palette[i].r,
                             palette[i].g,
                             palette[i].b);
      }
