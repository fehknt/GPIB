...
struct COMPLEX_DOUBLE      // No virtuals, so instances can be allocated with malloc() or new
{
   ...
   const DOUBLE hypot(void) const
      {
      return hypot(real, imag);
      }

   const DOUBLE carg(void) const
      {
      return atan2(imag, real);
      }

   const DOUBLE cmag(void) const
      {
      return sqrt(real*real+imag*imag);
      }

   const COMPLEX_DOUBLE cpow(COMPLEX_DOUBLE pwr) const
      {
   ...
   const COMPLEX_DOUBLE csqrt(void) const
      {
      return cpow(0.5);
      }
   ...
   const COMPLEX_DOUBLE conj(void) const
      {
      COMPLEX_DOUBLE temp;
