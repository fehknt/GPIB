...
struct SA_STATE
{
   ...
   S32     no_retrigger;         // If TRUE, do not trigger a new sweep before requesting trace data (some models only)
   ...
   S32    VSA_display_state;         // for E4406A
   S32    VSA_orig_avg_state;
   S32    VSA_orig_display_state;
         
   S32    VSA_zoomed;                // for 89400 series
   S32    VSA_n_record_samples;
   S32    VSA_a0;                    // alias-protected index range
   S32    VSA_a1;
   bool   VSA_vect_mode;
   DOUBLE VSA_input_Z;
...
SADEF SA_STATE * WINAPI  SA_get_state         (void);
