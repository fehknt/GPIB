      ...
      if (sd != INVALID_SOCKET)
         ...
         cout << "Accepted connection from " <<
              ...
              [&]() {
                  ...
                  return inet_ntop(AF_INET, &sinRemote.sin_addr, ip_buffer, sizeof(ip_buffer)) ? ip_buffer : "UnknownIP";
              }()
...
{   
   ...
   // Use inet_pton to convert string IP to binary form
   if (inet_pton(AF_INET, pcAddress, &sinInterface.sin_addr) == 1) {
       SOCKET sd = socket(AF_INET, SOCK_STREAM, 0);

       if (sd != INVALID_SOCKET)
          {
          // Original code had a redundant check for nInterfaceAddr != INADDR_NONE
          // and then created the socket. The logic is now integrated.
          // The sinInterface is already populated by inet_pton.

          // The original code used nInterfaceAddr directly, which was u_long.
          // sinInterface.sin_addr.s_addr = nInterfaceAddr; // This line is replaced by inet_pton
