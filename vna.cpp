...
bool is_8510(void)
{
   return (strstr(instrument_name,"8510") != NULL) ||
          (strstr(instrument_name,"8530") != NULL);
}
...
void instrument_setup(S32 SCPI, S32 SINGL, bool debug_mode = TRUE)
{
   if (SCPI)
      {
...
left at the default setting (OUTPDATA).  However, because OUTPDATA traces from the 85xx/87xx analyzers have undergone calibration correction but not post-processing functions such as electrical delay, port extensions, trace math, or time-domain gating, \
...
normalized).\n\nThe default reference resistance is 50 ohms.");
      ...
      case R_DB:
         {
         SetDlgItemText(hwnd, R_HELPTXT, "Select this option to save files with network parameters in dB-angle format (DB).\n\nIn this format, angles are given in degrees, while magnitudes are expressed as 20 * log10(magnitude).");
         break;
         }

      case R_MA:
         {
         SetDlgItemText(hwnd, R_HELPTXT, "Select this option to save files with network parameters in magnitude-angle format (MA).  Angles are given in degrees."); 
         break;
         }

      case R_RI:
         {
         SetDlgItemText(hwnd, R_HELPTXT, "Select this option to save files with network parameters in real-imaginary format (RI)."); 
         break;
         }

      case R_FREQ_FMT:
         {
         SetDlgItemText(hwnd, R_HELPTXT, "Specifies the desired frequency unit.\n\nFrequencies in Touchstone files are expressed in GHz by default, but some third-party applications may \
require frequency values to be expressed in Hz, kHz, or MHz instead.");
      ...
      case R_TCHECK:
         {
         SetDlgItemText(hwnd, R_HELPTXT, "Performs a mathematical check to assess VNA calibration and measurement uncertainty.  For more information, see Rohde && Schwarz Application Note 1EZ43, \"T-Check Accuracy Test for Vector Network Analyzers.\"\n\n\
The T-Check viewer is compatible with all VNAs that can export .S2P data files.  It may be launched independently from the Start menu program group. \n\n\
Note that the network analyzer must undergo a full two-port calibration with a properly-defined calibration kit prior to running the T-Check function.  Additionally, a T adapter with \
good symmetry and phase stability at the frequencies of concern is required.  The T adapter should be connected at the calibration reference plane using high-quality test cables (where applicable).\n\nSCPI analyzers, including FieldFox models, should be configured to display all four S-parameters prior to running T-Check.");
         break;
         }
...
under the Agilent/Keysight FieldFox series of portable VNAs, and only for saving Touchstone files.  It should be left unchecked when connecting to HP 85xx or 87xx models.\n\n\
...
When using an instrument that does not support the CALIONE2 calibration type, checking this box may cause lockups or other undefined behavior.");
      ...
      case R_SING:     
         {
         SetDlgItemText(hwnd, R_HELPTXT, "If this box is checked, a single-sweep command will be issued before retrieving S-parameter data from the analyzer. \
Otherwise, the analyzer's current data buffer contents will be saved without changing its sweep state.  \n\n\
Unchecking this box may be necessary in order to save averaged traces or other post-processed data associated with the OUTPFORM command.  Results may be \
unpredictable, especially when saving S parameters that are not currently being displayed.  Use caution when unchecking this box.");
         break;
         }
      ...
      default:
         {
         char buffer[256];
         _snprintf(buffer, sizeof(buffer), "VNA Utility V%s of %s by John Miles, KE5FX\nwww.ke5fx.com\n\nThis program may be copied and distributed freely.", VERSION, __DATE__);
         SetDlgItemText(hwnd, R_HELPTXT, buffer);
         break;
         }
      }
}
                ...
                bool include_cal)
{
   ...
   instrument_setup(SCPI, TRUE);    // TODO, need single sweep here?
              ...
              S32       SINGL)
{
   ...
   if (!SCPI)
      ...
      if (SnP == 1)
         {
         result = read_complex_trace(param, query, &S11[first_AC_point], n_AC_points, 50, SINGL);
         }
      else
         {
         result = read_complex_trace("S11", query, &S11[first_AC_point], n_AC_points, 20, SINGL) && 
                  read_complex_trace("S21", query, &S21[first_AC_point], n_AC_points, 40, SINGL) && 
                  read_complex_trace("S12", query, &S12[first_AC_point], n_AC_points, 60, SINGL) && 
                  read_complex_trace("S22", query, &S22[first_AC_point], n_AC_points, 80, SINGL);
         }
      }
   else
      {
...
//               {
//               if (param[1] == '1')
                  { result &= read_complex_trace_SCPI(p, &S11[first_AC_point], n_AC_points, pct, SINGL); pct += incr; }
//               else
...
//               }
            ...
            if (!_strnicmp(data,"S21",3))
               result &= read_complex_trace_SCPI(p, &S21[first_AC_point], n_AC_points, pct, SINGL); pct += incr;

            if (!_strnicmp(data,"S12",3))
               result &= read_complex_trace_SCPI(p, &S12[first_AC_point], n_AC_points, pct, SINGL); pct += incr;

            if (!_strnicmp(data,"S22",3))
               result &= read_complex_trace_SCPI(p, &S22[first_AC_point], n_AC_points, pct, SINGL); pct += incr;
            }
         }
      }
      ...
      case WM_COMMAND:
         ...
         switch (id)
            ...
            case R_SAVE:
               ...
               S32 SINGL = Button_GetCheck(GetDlgItem(hwnd, R_SING)); 
            ...
            case R_SAVE_S:
               ...
               save_state(addr, SCPI, FALSE, SINGL, FALSE);
               break;
               }
            ...
            case R_SAVE_SC:
               ...
               save_state(addr, SCPI, CALIONE2, SINGL, TRUE);
               break;
               }
                   ...
                   LPSTR _lpszCmdLine, int nCmdShow)
{
   ...
   Button_SetCheck(GetDlgItem(hwnd, R_SING), TRUE);   
