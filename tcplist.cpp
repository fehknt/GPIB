...
void main(S32 argc, C8 **argv)
{
   if (argc < 2)
      ...
      printf("               [\"string to transmit\"] [wait_msec] [xmit_delay_ms] [output_to_file.txt]\n");
      printf("               [offset_value]\n");
      printf("   - EOS_character defaults to 10\n");
      printf("   - repeat_cnt defaults to 1 (0=repeat until interrupted)\n");
      printf("   - timeout_ms defaults to 0 (no timeout)\n");
      printf("   - wait_ms defaults to 0 (do not wait before connecting)\n");
      printf("   - xmit_delay_ms defaults to 0 (no delay after transmitting)\n");
      printf("   - Press ESC to terminate connection\n\n");
      printf("Example: tcplist 192.168.1.221:5025 10 1 1000 \"*IDN?\" 1000\n");
      printf(" yields: Agilent Technologies,34461A,MY53201222,A.03.00-02.40-03.00-00.52-01-01\n");
      exit(1);
      }
   ...
   if (argc >= 8) xmit_wait_ms = atoi(argv[7]);
   if (argc >= 10) sscanf(argv[9],"%lf",&offset_value);

   FILE *outfile = NULL;

   if (argc >= 9)
      {
      outfile = fopen(argv[8],"wt");
      setbuf(outfile, NULL);
      atexit(shutdown);
      }
