...
public:
   ...
   static C8 *address_string(sockaddr_in *address, 
                             C8          *text, 
                             S32          text_array_size,
                             // Original type was bool, restoring it.
   ...
   static bool parse_address(C8 *IP_address, S32 default_port, sockaddr_in *dest)
      ...
      // The original code had dest->sin_family = PF_INET; dest->sin_port = htons((U16) server_port_num);
      // These are now handled by getaddrinfo.
   ...
   static bool hostname_string(sockaddr_in *address, 
      ...
      // Using getnameinfo for reverse DNS lookup
      // This is a blocking call, so the timeout logic needs to be external or handled differently.
      // The original code used WSAAsyncGetHostByAddr with a polling loop, which is deprecated.
      // For now, I'll make it blocking as getnameinfo is typically blocking.
      // If a non-blocking reverse DNS is strictly needed, it would require a separate thread.
      ...
      S32 end   = 0;
      end = timeGetTime(); // Simulate immediate completion for blocking call
      ...
      if (status == 0) {
          ...
          // Original code appended (ms) which is not directly supported by getnameinfo timeout
          // If timeout is critical, this function needs to be refactored to use a separate thread.
          return TRUE;
      } else {
          // Failed to resolve hostname, fall back to numeric IP
          // getnameinfo can return EAI_AGAIN, EAI_FAIL, EAI_NONAME, etc.
          // For now, just use the numeric address string.
   ...
   virtual void connect(C8  *IP_address,                              
      ...
      hints.ai_flags = AI_PASSIVE;      // For local address
      ...
      if (getaddrinfo(local_hostname, NULL, &hints, &res) == 0) {
          for (p = res; p != NULL; p = p->ai_next) {
              if (p->ai_family == AF_INET) {
                  inaddrIP = ((struct sockaddr_in*)p->ai_addr)->sin_addr; // Corrected to use sin_addr
                  break;
              }
          }
          freeaddrinfo(res);
      ...
      char ip_buffer[INET_ADDRSTRLEN];
      if (inet_ntop(AF_INET, &inaddrIP, ip_buffer, sizeof(ip_buffer)) != NULL) { // Use inet_ntop
          strcpy(ipnum, ip_buffer);
      } else {
          strcpy(ipnum, "Unknown"); // Fallback
      }
     
      message_printf(IP_VERBOSE, "Using Windows Sockets V%d.%d (%s)",
         HIBYTE(wsadata.wVersion), // Corrected to use wsadata.wVersion
   ...
   virtual void server_accept(sockaddr_in *who, 
      ...
      // Using address_string which now uses inet_ntop.
      //
   ...
   virtual bool read_text_line(C8       *incoming_line,
                               ...
                               S32       timeout_msec       = 3000,    // 0 = no timeout // Changed from S32 to const S32
                               S32       dropout_msec       = 3000,
                               S32       EOS                = 10)
      {                   
      if (!active)
...
public:
   ...
   virtual bool set_remote_address(const C8  *IP_address,
      ...
      message_printf(IP_NOTICE, "UDP remote address: %s:%d", // Use inet_ntop
              [&]() {
                  char ip_buffer_local[INET_ADDRSTRLEN];
   ...
   // compatibility with the TCP/IP version.) // Removed virtual
   //
