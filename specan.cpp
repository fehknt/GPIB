...
#define ASCII_ACQ                                                              \
  0 // 1 to use ASCII mode to fetch data, when supported (very slow in some
    // cases)
...
//***************************************************************************
//
// Like strstr() but case-insensitive (and can optionally treat hyphens and
// underscores as equal)
//
//***************************************************************************

static C8 *stristr(C8 *str1, const C8 *str2,
                   bool underscores_match_hyphens = FALSE) {
  C8 *result = NULL;
...
void __cdecl alert(C8 *caption, C8 *fmt, ...) {
  ...
  _vsnprintf(state.error_text, sizeof(state.error_text) - 1, fmt, ap);
...
void WINAPI GPIB_error(GPIBPORT *P, C8 *msg) {
  if (state.error) {
    return;
  }
...
bool GPIB_connect(
    S32 device_address, GPIBERR handler, bool clear = FALSE,
    S32 timeout_msecs = 100000, // use -1 to disable timeout checks
    S32 board_address = -1, bool release_system_control = FALSE,
    ...
    S32 force_auto_read = -1)       // -1 = use INI_force_auto_read setting
{
  ...
  if (!GPIB.connect(device_address, handler, clear, timeout_msecs,
                    board_address, release_system_control, max_read_buffer_len,
                    FALSE)) {
    return FALSE;
  }
...
S32 GPIB_set_EOS_mode(S32 new_EOS_char, bool send_EOI_at_EOT = TRUE,
                      bool enable_board_configuration = TRUE) {
  if (state.error) {
    return -1;
  }

  return GPIB.set_EOS_mode(new_EOS_char, send_EOI_at_EOT,
...
C8 *GPIB_query(C8 *string, bool from_board = FALSE) {
  temp_buffer[0] = 0;
...
C8 *GPIB_read_ASC(S32 max_len = -1, bool report_timeout = TRUE,
...
C8 *GPIB_read_BIN(S32 max_len = -1, bool report_timeout = TRUE,
                  bool from_board = FALSE, S32 *actual_len = NULL) {
  temp_buffer[0] = 0;
...
void __cdecl alert(C8 *caption, C8 *fmt, ...) {
  ...
  if (caption == NULL) {
    MessageBox(NULL, state.error_text, "Error", MB_OK);
  } else {
    MessageBox(NULL, state.error_text, caption, MB_OK);
  }
}

void fail(S32 exit_code) { exit(exit_code); }

static GPIBERR user_error_callback = NULL;

void WINAPI GPIB_error(C8 *msg, S32 ibsta, S32 iberr, S32 ibcntl) {
  if (user_error_callback != NULL) {
    user_error_callback(msg, ibsta, iberr, ibcntl);
  } else {
...
bool WINAPI GPIB_connect_to_analyzer(S32 address) {
  ...
  {
    GPIB_connect(address, GPIB_error, 0, disable_GPIB_timeouts ? -1 : 10000);
    ...
    //
    // Because we can't read acknowledgements from the Tek 49x analyzers, we
    // need to flush the Prologix board's receive buffers explicitly
    //
  ...
  //
  // Is this an auto-detectable instrument?  If not, special-case it based on
  ...
  if (state.SA44_mode) {
    ...
    alert("GPIB_connect_to_analyzer() error",
          "Signal Hound not supported in 64-bit Windows");
    return FALSE;
#else
    ...
    if (!state.SH_initialized) {
      ...
      //
      // Set CWD to .exe's path so SHAPI_Initialize() will find Hound2Pipe.exe,
      // then restore it (Not thread-safe!)
      //
      ...
      if (err) {
        ...
        alert("SHAPI_Initialize() error",
              "Signal Hound not found, code %d\n\n(If the Signal Hound is "
              "currently plugged in, try disconnecting and reconnecting it.)",
              err);
        return FALSE;
      }
    ...
    if (state.CTRL_span_Hz >= 0.0) {
      if (state.CTRL_span_Hz < 1.0) {
        ...
        alert(
            "Error",
            "Zero span not supported -- must specify a valid nonzero span/div");
        return FALSE;
      }
    }
    ...
    S32 err = SHAPI_Configure(atten_dB, above_150_MHz, sens, decim, IF_path,
                              ADC_clock);
    if (err) {
      SA_disconnect();
      state.type = SATYPE_INVALID;

      alert("SHAPI_Configure() error",
            "Signal Hound could not be configured, code %d", err);
      return FALSE;
    }

    S32 cnt = SHAPI_GetFastSweepCount(start_Hz, stop_Hz, FFT_size);
    ...
    if ((cnt > MAX_N_PTS) || (cnt < 0)) {
      ...
      alert("Error",
            "Specified parameters result in excessive span buffer size\n(%u "
            "points, up to %d allowed)",
            cnt, MAX_N_PTS);
      return FALSE;
    }
    ...
    while (state.dB_division == 10000) {
      ...
      if (state.dB_division == 0) {
        ...
        alert("Error", "LIN mode not supported -- must select logarithmic "
                       "vertical display (dB/div)");
        return FALSE;
      }
    }
    ...
    while (span == -10000.0) {
      ...
      if (span < 0.0) {
        ...
        alert("Error",
              "Must select a valid numeric span (PER DIV or 1.7-22 GHz)");
        return FALSE;
      }
    }
    ...
    SA_printf("SV4"); // Disable autocalibration (which prevents end-of-sweep
                      // bit from going high)
    SA_printf("FA");  // Force start/stop annotation mode
    SA_printf("S2");  // Select single-sweep mode

    //
    // Perform "D7T4" query to obtain display annotation with immediate
    // triggering (EOS is set to 10 to separate the strings)
    //
    ...
    if (strstr(annotation[5], "GHZ"))
      state.min_Hz *= 1.0E9;
    else if (strstr(annotation[5], "MHZ"))
      state.min_Hz *= 1.0E6;
    else if (strstr(annotation[5], "KHZ"))
      state.min_Hz *= 1.0E3;

    if (strstr(annotation[6], "GHZ"))
      state.max_Hz *= 1.0E9;
    else if (strstr(annotation[6], "MHZ"))
      state.max_Hz *= 1.0E6;
    else if (strstr(annotation[6], "KHZ"))
      state.max_Hz *= 1.0E3;
    ...
    for (i = 0; i < 8;
         i++) // Skip garbage characters (08 20) appearing before scale text
    {
    ...
    if (strstr(OT_buffer[9], "GHz"))
      state.min_Hz *= 1.0E9;
    else if (strstr(OT_buffer[9], "MHz"))
      state.min_Hz *= 1.0E6;
    else if (strstr(OT_buffer[9], "kHz"))
      state.min_Hz *= 1.0E3;

    if (strstr(OT_buffer[10], "GHz"))
      state.max_Hz *= 1.0E9;
    else if (strstr(OT_buffer[10], "MHz"))
      state.max_Hz *= 1.0E6;
    else if (strstr(OT_buffer[10], "kHz"))
      state.max_Hz *= 1.0E3;
    ...
    SA_printf("SRDR");
    GPIB_flush_receive_buffers(); // if we don't do this, the queries below
                                  // return one string behind (from PN)

    sprintf(state.ID_string,
            "TR4172/TR4173"); // seems to have been made by at least 3
                              // manufacturers? (T-R, Advantest, Anritsu)
    ...
    GPIB_set_EOS_mode(
        10, TRUE); // (Under Prologix, ASCII reads won't terminate on EOI
                   // reception, since we never set ++eot_enable and ++eot_char)

    GPIB_puts("DL0 SI S1 S2 HD0"); // Expect (and return) CR+LF|EOI delimiters,
                                   // use single-sweep mode, disable SRQs, and
                                   // clear status byte
    GPIB_flush_receive_buffers();  // (Needed on R3271A at least)

    GPIB_puts("TYP?");
    sprintf(state.ID_string, "Advantest %s",
            GPIB_read_ASC()); // (Reads 'R3361A\r\r\n' under Prologix)
    ...
    if ((strstr(state.ID_string, "3261") != NULL) ||
      ...
      GPIB_puts("TPC"); // Select 401-point vertical resolution for
                        // compatibility with R3261/R3361
    }
    ...
    default:
      state.dB_division = 0;
      break; // (we don't support 0.5 dB/div)
    }
    ...
    if ((state.max_dBm == 10000.0) || (state.dB_division == 0)) {
      ...
      alert("Error",
            "Must select logarithmic vertical display in dBm (1-10 dB/div), "
            "'%s' returned",
            DD);
      return FALSE;
    }
    ...
    if (state.type == SATYPE_R3265_R3271) {
      ...
      //
      // R3261/R3361 support 8/12 division displays at 10 dB/division -- find
      // out which
      //
    ...
    //
    // (Note, however, that ++spoll results will still come back with CR/LFs
    // added by the Prologix firmware)
    //
  ...
  fprintf(stderr, "SA_connect: Entering ID query loop...\n");
  do {
    if ((timeGetTime() - start_time) > 10000) {
      fprintf(stderr, "SA_connect: ID query timeout!\n");
      GPIB_disconnect();
      alert("Error", "No valid response to ID? or *IDN? query");
      return FALSE;
    }

    if (state.SCPI_mode || state.HP358XA_mode ||
        state.Advantest_mode) // TODO: -advantest should just set SCPI mode?
    {
      if (state.ID_string[0] == 0) {
        fprintf(stderr, "SA_connect: Querying *IDN?...\n");
        strcpy(state.ID_string, GPIB_query("*IDN?"));
      }
    } else {
      if (state.ID_string[0] == 0) {
        fprintf(stderr, "SA_connect: Querying ID?...\n");
        strcpy(state.ID_string, GPIB_query("ID?"));
      }
    }

    fprintf(stderr, "SA_connect: Got ID string: [%s]\n", state.ID_string);

    for (S32 i = strlen(state.ID_string) - 1; i >= 0; i--) {
      if (!isspace((U8)state.ID_string[i]))
        break;
      state.ID_string[i] = 0;
    }
  } while (strlen(state.ID_string) <= 3);

  fprintf(stderr, "SA_connect: Final ID string: [%s]\n", state.ID_string);
  ...
  if ((!_strnicmp(state.ID_string, "ID TEK/49", 9)) ||
    ...
    SA_printf("RLUNIT DBM;"); // Units in dBm (not supported on 1984-level
                              // instruments)
    ...
    while (span == -10000.0) {
      ...
      if (strstr(text, "SPAN MAX") != NULL) {
        span = FLT_MAX;
        SA_fetch_trace(); // Get min/max frequency from trace header in MAX SPAN
                          // mode...
    ...
    if (span < 1.0) // Firmware bug on all 49x versions returns meaningless
                    // CURVE data in zero-span mode
    {               // (also ~30 dB too low in bands > 60 GHz?)
      SA_disconnect();
      state.type = SATYPE_INVALID;

      alert("Error",
            "ZERO SPAN not supported -- must select a valid nonzero span/div");
      return FALSE;
    }

    if (state.min_Hz ==
        -FLT_MAX) // i.e., if not determined by SA_fetch_trace() call above
    {
    ...
    if (strstr(vflt, "WIDE") != NULL) {
      if (state.RBW_Hz >= 300000)
        state.VBW_Hz = 30000.0;
      else if (state.RBW_Hz >= 30000)
        state.VBW_Hz = 3000.0;
      else if (state.RBW_Hz >= 3000)
        state.VBW_Hz = 300.0;
      else if (state.RBW_Hz >= 300)
        state.VBW_Hz = 30.0;
      else
        state.VBW_Hz = 3.0;
    ...
    while (state.dB_division == 10000) {
      ...
      if ((strstr(text, "LIN") != NULL) || (strstr(text, "FM") != NULL) ||
    ...
    SA_printf("RQS 1;"); // Enable "end of sweep" bit in status byte
    SA_printf("VAV 0;"); // Disable video averaging

    //
    // Acquire from trace A by default
    //

    if (!state.trace_cmd[0]) {
      strcpy(state.trace_cmd, "TA");
    }
  ...
  } else if (((!_strnicmp(state.ID_string, "ID TEK/278", 10))) ||
             ((!_strnicmp(state.ID_string, "TEK/278", 7)))) {
    ...
    while (state.vid_avgs < 0) {
      ...
      if (strstr(text, "off") !=
          NULL)             // (8561E always reports 100 after PRESET, even
    ...
    SA_printf(
        "DL0;SI"); // Select CR+LF+EOI query termination and single-sweep mode
    GPIB_flush_receive_buffers();
    ...
    //
    // R3465/R3132 can be set to either 501 or 1001 points, but on the R3465
    // family there's no TP? query; the default is 1001 points.  R3131 supports
    // 501 points only
    //
    ...
    state.n_trace_points =
        (state.hi_speed_acq || state.type == SATYPE_R3131) ? 501 : 1001;
    ...
    if (strstr(GPIB_query("PNLMD?"), "SPECT") == NULL) {
      ...
      alert("Error",
            "Instrument does not appear to be in spectrum analyzer mode");
      return FALSE;
    }
    ...
    SA_printf("TRM 0;BIN 1");
    GPIB_flush_receive_buffers();
  } else if (state.SCPI_mode || state.HP358XA_mode ||
    ...
    // need to identify it separately.  Likewise, the R&S FSE-series analyzers
    // are not identical in operation to the Agilent E4400 series, so a
    // distinction must be made.  The same is true of the specialized E4406A
    // transmitter tester.
    //
    // (Use of *OPC? query at the end of every non-query command string
    // eliminates unterminated-query warning messages seen with Prologix boards)
    //
    ...
    if ((strstr(state.ID_string, "3588A") != NULL) ||
      ...
      if (strstr(GPIB_query(":INST:SEL?"), "BASIC") == NULL) {
        ...
        alert(
            "Error",
            "Instrument does not appear to be in BASIC spectrum analyzer mode");
        return FALSE;
      }

      state.VSA_orig_display_state = (GPIB_query(":DISP:ENAB?")[0] != '0');
      state.VSA_orig_avg_state =
          (GPIB_query(":SENS:SPEC:AVER:STAT?")[0] != '0');

      state.VSA_display_state = state.VSA_orig_display_state;

      GPIB_query(":SENS:SPEC:AVER:STAT 0;*OPC?"); // Disable averaging
      GPIB_query(
          ":DISP:FORM:ZOOM1;*OPC?"); // Zoom to spectrum analyzer display window
      state.type = SATYPE_E4406A;
    } else if ((strstr(state.ID_string, "89410") != NULL) ||
               (strstr(state.ID_string, "89440") != NULL) ||
               (strstr(state.ID_string, "89441") != NULL)) {
      //
      // On HP 89400A series, SENS:SWEEP:POINTS? returns the number of points in
      // the visible sweep, which is the alias-protected subset, e.g. 401 by
      // default.
      //
      // :SENS:FREQ:BAS? returns 0 if zoomed operation is in effect, but only in
      // vector mode. Trace data is always complex if INST:SEL? returns VECT
      //
      ...
      while (mode == 0) {
        C8 *text = GPIB_query(":INST:NSEL?");
        sscanf(text, "%d", &mode);
      }

      switch (mode) {
      case 1:
        state.VSA_vect_mode = FALSE;
        break; // SCALar
      case 3:
        state.VSA_vect_mode = TRUE;
        break; // VECTor

      default: {
        alert("Error",
              "VSA must be configured in standard scalar or vector mode");
        return FALSE;
      }
      }
      ...
      if (state.VSA_vect_mode) {
        //
        // In vector mode, we can query the zoom state with :SENS:FREQ:BASeband?
        // This always returns +1 in scalar mode, though, even when zoomed
        //

        state.VSA_zoomed = -1;

        while (state.VSA_zoomed == -1) {
          C8 *text = GPIB_query(":SENS:FREQ:BAS?");
          sscanf(text, "%d", &state.VSA_zoomed);
        }

        state.VSA_zoomed = !state.VSA_zoomed;
      } else {
        //
        // In scalar mode, there seems to be no way to tell whether or not the
        // measurement is "zoomed" except by looking at the number of points
        // returned in the trace header. TRACe:DATA:HEADer:POINts? D1 returns
        // the number of points that were copied to the D1 register, which will
        // be the next larger value of (2^n)+1, or simply 2^n for zoomed data.
        // See table on page 447 of GPIB manual.  Presumably we could use this
        // method in both scalar and vector modes, more testing needed
        //

        GPIB_query("TRACE:COPY D1,TRACE1;*OPC?");

        state.VSA_n_record_samples = 0;

        while (state.VSA_n_record_samples == 0) {
          C8 *text = GPIB_query(":TRACE:DATA:HEADER:POINTS? D1");
          sscanf(text, "%d", &state.VSA_n_record_samples);
        }

        state.VSA_zoomed = (state.VSA_n_record_samples & 1) == 0;
      }

      //
      // Regardless of scalar/vector or zoomed/unzoomed mode, SENS:SWEEP:POINTS
      // always reflects the number of valid, alias-protected samples that will
      // ultimately be captured. TRACe:DATA:HEADer:POINts?  will always return
      // the next higher power of 2, plus 1 if zoomed
      //

      state.VSA_n_record_samples = 0;
      state.VSA_a0 = 0;
      state.VSA_a1 = state.n_trace_points - 1;

      switch (state.n_trace_points) {
      case 51:
        if (state.VSA_zoomed) {
          state.VSA_n_record_samples = 64;
          state.VSA_a0 = 7;
          state.VSA_a1 = 57;
        } else
          state.VSA_n_record_samples = 65;
        break;
      case 101:
        if (state.VSA_zoomed) {
          state.VSA_n_record_samples = 128;
          state.VSA_a0 = 14;
          state.VSA_a1 = 114;
        } else
          state.VSA_n_record_samples = 129;
        break;
      case 201:
        if (state.VSA_zoomed) {
          state.VSA_n_record_samples = 256;
          state.VSA_a0 = 28;
          state.VSA_a1 = 228;
        } else
          state.VSA_n_record_samples = 257;
        break;
      case 401:
        if (state.VSA_zoomed) {
          state.VSA_n_record_samples = 512;
          state.VSA_a0 = 56;
          state.VSA_a1 = 456;
        } else
          state.VSA_n_record_samples = 513;
        break;
      case 801:
        if (state.VSA_zoomed) {
          state.VSA_n_record_samples = 1024;
          state.VSA_a0 = 112;
          state.VSA_a1 = 912;
        } else
          state.VSA_n_record_samples = 1025;
        break;
      case 1601:
        if (state.VSA_zoomed) {
          state.VSA_n_record_samples = 2048;
          state.VSA_a0 = 224;
          state.VSA_a1 = 1824;
        } else
          state.VSA_n_record_samples = 2049;
        break;
      case 3201:
        if (state.VSA_zoomed) {
          state.VSA_n_record_samples = 4096;
          state.VSA_a0 = 448;
          state.VSA_a1 = 3648;
        } else
          state.VSA_n_record_samples = 4097;
        break;

      default: {
        alert("Error", "Instrument must be configured for 51, 101, 201, 401, "
                       "801, 1601, or 3201 points");
        return FALSE;
      }
      }

      //
      // Determine input Z used for trace data conversion to dBm
      //
      // 89400 input load is set via INP:IMP, but in 1M mode, dBm can optionally
      // be computed based on an arbitrary user-entered impedance
      //

      DOUBLE Zo = 0.0;

      while (Zo == 0.0) {
        C8 *text = GPIB_query(":INP:IMP?");
        sscanf(text, "%lf", &Zo);
      }
      ...
      if (Zo == 1E6) {
        ...
        while (Zo == 0.0) {
          C8 *text = GPIB_query(":SENS:CORR:IMP:INP:MAGN?");
          sscanf(text, "%lf", &Zo);
        }
      }

      state.VSA_input_Z = Zo;

      state.type = SATYPE_HP89400;
    } else {
      GPIB_query("*SRE 0;*CLS;:INIT:CONT 0;*OPC?");
    ...
    if (state.type == SATYPE_E4406A) {
      ...
      //
      // NB: in I/Q spectrum mode, :SENS:FREQ:CENT? returns 1.000000000E+009,
      // while the :SPAN? query is correct
      //
    ...
    } else if ((state.type == SATYPE_FSE) || (state.type == SATYPE_FSU) ||
               (state.type == SATYPE_FSP)) {
      if (state.type == SATYPE_FSP)
        GPIB_query(":SENS:SWE:POIN 501;*OPC?"); // FSP and FSU point counts are
                                                // configurable, but we only
                                                // support the default
      else if (state.type == SATYPE_FSU)
        GPIB_query(":SENS:SWE:POIN 625;*OPC?"); // (TODO: should set FSE to 500
                                                // as well?)
      ...
      if (_strnicmp(text, "LOG", 3)) {
        ...
        alert("Error",
              "Must select logarithmic vertical display (1-10 dB/div)");
        return FALSE;
      }
    ...
    } else if ((state.type == SATYPE_N9900) || (state.type == SATYPE_HP89400)) {
      //
      // Query N9900- or 89400-series reference level and scale
      //
      ...
      if (((state.dB_division < 1) || (state.dB_division > 10)) ||
          ((state.type == SATYPE_HP89400) &&
           (strstr(GPIB_query("CALC:FORM?"), "MLOG") == NULL))) {
        SA_disconnect();
        state.type = SATYPE_INVALID;
      ...
      while (state.dB_division == 10000) {
        DOUBLE fdB = 0.0;
        C8 *text =
            GPIB_query(state.type == SATYPE_SCPI ? ":DISP:WIND:TRAC:Y:PDIV?"
                                                 : ":DISP:Y:SCAL:PDIV?");
        sscanf(text, "%lf", &fdB);
        state.dB_division = (S32)(fdB + 0.5F);
      }
      ...
      while (state.max_dBm == 10000.0) {
        C8 *text =
            GPIB_query(state.type == SATYPE_SCPI ? ":DISP:WIND:TRAC:Y:RLEV?"
                                                 : ":DISP:Y:SCAL:MAX?");
        sscanf(text, "%lf", &state.max_dBm);
      }
    ...
    //
    // Request 32-bit little-endian integer trace format for E4400-style SCPI,
    // or 32-bit IEEE754 reals for HP 3588A/3589A, E4406A, 894xx, FSx, and the
    // N9900 series handhelds
    //
    ...
    if (state.type == SATYPE_HP358XA) {
      ...
      GPIB_query(
          ":FORM:DATA REAL,32;:FORM:BORD NORM;*OPC?"); // NORM is LE on N9900
    } else if (state.type == SATYPE_E4406A) {
      GPIB_query(
          ":FORM:DATA REAL,32;:FORM:BORD SWAP;*OPC?"); // SWAP is LE on E4406A
                                                       // (and presumably other
                                                       // SCPI instruments...?
                                                       // TODO)
    } else if (state.type == SATYPE_HP89400) {
      GPIB_query(":FORM:DATA REAL,32;*OPC?"); // 89400 series is always BE
    } else {
    ...
    if (state.type == SATYPE_HP358XA) {
      ...
      state.n_trace_points = 625;
    } else if (state.type == SATYPE_FSP) {
      state.n_trace_points = 501;
    } else if (state.type == SATYPE_E4406A) {
      //
      // E4406A VSA appears to have no way to query the # of points that will
      // actually be returned by a :FETCH:SPEC4 query, so we must request a
      // trace and see how many points it gives us
      //
      ...
      //
      // Generic point count query for other SCPI instruments
      //
...
//
// (What the 494AP actually returns is 0xff 0x0d 0x0a, for some unknown
// reason...)
//
//****************************************************************************
...
void __cdecl SA_printf(C8 *fmt, ...) {
  ...
  if ((state.type == SATYPE_TEK_490P) || (state.type == SATYPE_TEK_271X)) {
    GPIB_read_ASC();
  }
}
...
void WINAPI SA_parse_command_line(C8 *command_line) {
  ...
  for (;;) {
    ...
    //
    // -trace: GPIB command to be transmitted to retrieve trace data
    // (currently supported only by 8566B/8568B)
    //

    option = strstr(lpCmdLine, "-trace:\"");
    ...
    option = stristr(lpCmdLine, "-856XA");
    ...
    option = stristr(lpCmdLine, "-358XA");
    ...
    option = stristr(lpCmdLine, "-8569B");
    ...
    //
    // -advantest: Enable Advantest R3264/R3267/R3273, R3463/R3465/R3272/R3263,
    // or R3131/R3132/R3162/R3172/R3182 mode
    //
    // (i.e., any R31xx, R32xx, or R34xx analyzer that recognizes *IDN query)
    //

    option = stristr(lpCmdLine, "-ADVANTEST");
    ...
    option = stristr(lpCmdLine, "-R3261");
    ...
    //
    // -f: Enable fastest possible acquisition (lower resolution or precision,
    // disable screen, etc.)
    //
    ...
    //
    // -G: Inhibit sweep trigger prior to trace download (FSP only for now)
    //

    option = strstr(lpCmdLine, "-G");

    if (option != NULL) {
      state.no_retrigger = TRUE;
      memmove(option, &option[2], strlen(&option[2]) + 1);
      continue;
    }

    //
    // -RL, -CF, -span, -start, -stop, -bins, -RFATT, -sens, -divs, -logdiv:
    // Signal Hound control options
    //

    option = stristr(lpCmdLine, "-RL");
    ...
    option = stristr(lpCmdLine, "-START");
    ...
    option = stristr(lpCmdLine, "-RFATT");
    ...
    option = stristr(lpCmdLine, "-LOGDIV");
...
bool WINAPI SA_connect(S32 address, C8 *command_line, GPIBERR error_callback) {
  user_error_callback = error_callback;

  if (command_line != NULL) {
    SA_parse_command_line(command_line);
  }
  bool result = TRUE;
  ...
  {
    strcpy(state.ID_string,
           ""); // (Default "Test mode" string causes crash in -scpi mode)
...
void WINAPI SA_disconnect(bool final_exit) {
  ...
  if (state.type == SATYPE_TEK_490P) {
    ...
    SA_printf(
        "TB0S1RC4"); // Hide trace 'B', continuous sweep, re-enable autocal
    ...
    SA_printf("SN;DL0"); // Restore original resolution, continuous sweep,
                         // default <CR><LF><EOI> response delimiter
  } else if ((state.type == SATYPE_R3465_SERIES) ||
             (state.type == SATYPE_R3132_SERIES)) {
    SA_printf("TPL;SN;DL0"); // Restore default 1001-point mode, continuous
                             // sweep, default <CR><LF><EOI> response delimiter
  } else if ((state.type == SATYPE_R3261_R3361) ||
             (state.type == SATYPE_R3265_R3271)) {
    GPIB_auto_read_mode(TRUE);
    SA_printf("SN FR DL0"); // Normal sweep, free-run, default <CR><LF><EOI>
                            // response delimiter
    ...
    GPIB_query("CONTS;");
  } else if ((state.type == SATYPE_FSE) || (state.type == SATYPE_FSP) ||
             (state.type == SATYPE_FSU) || (state.type == SATYPE_N9900) ||
             (state.type == SATYPE_HP89400) || (state.type == SATYPE_SCPI)) {
...
// -------------------------------------------------------------------------
// SA_query_sweep_time() - Internal helper to query sweep time with retries
// for instruments that might be busy or slow to compute it (like HP at narrow RBW)
// -------------------------------------------------------------------------

static DOUBLE SA_query_sweep_time(C8 *query_cmd) {
  DOUBLE swp_time = 0.0;
  GPIB_set_EOS_mode(10, FALSE);
  for (S32 retries = 0; retries < 120; retries++) {
    SA_printf(query_cmd);
    C8 *res = GPIB_read_ASC(-1, FALSE);
    if (res && strlen(res) > 0) {
      swp_time = atof(res);
      if (swp_time >= 0.0)
        break;
...
void WINAPI SA_fetch_trace(void) {
  ...
  if (state.type == SATYPE_SA44) {
    ...
    SHAPI_GetFastSweep(trace, state.min_Hz, state.max_Hz,
                       n_expected_trace_points, state.FFT_size,
    ...
    if (state.blind_data == NULL) {
      ...
      if (pts == 1000) {
        ...
        preamble_header =
            "WFMPRE "
            "WFID:FULL,ENCDG:BIN,NR.PT:%d,PT.FMT:Y,PT.OFF:%d,XINCR:%lf,XZERO:%"
            "lf,XUNIT:HZ,YOFF:%lf,YMULT:%lf,YZERO:%lf";
        strcpy(P->expected_curve, "CURVE CRVID:FULL,%");
      } else {
        SA_printf("WFMPRE WFID:A,ENC:BIN;");
        preamble_header = "WFMPRE "
                          "WFID:A,ENCDG:BIN,NR.PT:%d,PT.FMT:Y,PT.OFF:%d,XINCR:%"
                          "lf,XZERO:%lf,XUNIT:HZ,YOFF:%lf,YMULT:%lf,YZERO:%lf";
      ...
      sscanf(preamble, preamble_header, &P->nr_pt, &P->pt_off, &P->xincr,
             &P->xzero, &P->yoff, &P->ymult, &P->yzero);
...
#else
    S32 actual =
        0; // display actual trailing character(s) for diagnostic purposes
    ...
    for (S32 p = 0; p < pts; p++) {
      ...
      trace[p] =
          (P->yzero + state.A_offset) + (P->ymult * (S32(curve[p]) - P->yoff));
    }
    ...
    if (P == NULL) {
      ...
      C8 *preamble_header =
          "WFMPRE "
          "WFID:A,ENCDG:BIN,NR.PT:%d,PT.FMT:Y,PT.OFF:%d,XINCR:%lf,XZERO:%lf,"
          "XUNIT:HZ,YOFF:%lf,YMULT:%lf,YZERO:%lf";
    ...
    //
    // Wait for sweep to complete (bit 0 of status byte on 8566B/8568B)
    //
    Sleep(100);
    ...
    U8 stb = 0;
    S32 count = 0;
    while (!((stb = GPIB_serial_poll()) & 0x01)) {
      FILE *f = fopen(
          "c:\\Users\\fehkn\\OneDrive\\Documents\\GitHub\\KE5FX\\GPIB\\specan_"
          "sync_log.txt",
          "a");
      if (f) {
        if ((count++ % 10) == 0)
          fprintf(f, "Waiting for sweep... STB: 0x%02X\n", stb);
        if ((timeGetTime() - start_time) > 120000) {
          fprintf(f, "Sweep wait timeout! Last STB: 0x%02X\n", stb);
          fclose(f);
          break;
        }
        fclose(f);
    ...
    FILE *f = fopen(
        ...
        "a");
    if (f) {
      fprintf(f, "Sweep wait finished. STB: 0x%02X, Time: %d ms\n", stb,
              (S32)(timeGetTime() - start_time));
      fclose(f);
    }

#if ASCII_ACQ
    SA_printf("O3;%s",
              state.trace_cmd); // Request ASCII dBm values from specified trace
                                // (default = TA) (Slow)
    for (S32 i = 0; i < 1001; i++) {
      do {
        C8 *read_result = GPIB_read_ASC(16, FALSE);
    ...
    if (!state.hi_speed_acq) {
      SA_printf(
          "O2;%s",
          state.trace_cmd); // Request 16-bit binary display values from
                            // specified trace (default = TA) (Slightly slower
                            // but more precise than 8-bit binary mode)
      static U16 curve[1001];
      ...
      SA_printf(
          "O4;%s",
          state.trace_cmd); // Request 8-bit binary display values from specfied
                            // trace (default = TA) (Fast, but less precise)
      static U8 curve[1001];
    ...
    if (!state.hi_speed_acq) {
      SA_printf("O2;TROUT TRNOR"); // Request 16-bit binary display values from
                                   // trace A (Slightly slower but more precise
                                   // than 8-bit binary mode)
      static U16 curve[1001];
      ...
      SA_printf("O4;TROUT TRNOR"); // Request 8-bit binary display values from
                                   // trace A (Fast, but less precise)
      static U8 curve[1001];
    ...
    //
    // HP 8560 uses SWP? for sweep time
    //

    static U16 curve[601];
    static DOUBLE trace[601];

    DOUBLE swp_time = SA_query_sweep_time("SWP?;");
    
    SA_printf("CLRW TRA;TS;");
    Sleep((S32)(swp_time * 1000.0) + 500);

    SA_printf(
        "TDF B;TRA?"); // Request 16-bit binary display values from trace A

    memset(curve, 0, 601 * sizeof(S16));
    memcpy(curve, GPIB_read_BIN(601 * sizeof(S16), TRUE, FALSE),
           601 * sizeof(S16));
    ...
    // Take sweep, and acquire 16-bit binary data from display trace A
    //
    // HP 8590 uses ST? for sweep time
    //
    ...
    // Take sweep, and acquire 16-bit binary data from display trace A
    //
    // HP 70000 uses SWP? for sweep time
    //
    ...
    SA_printf("MDS W;TDF B;TRA?"); // Request 16-bit binary measurement uints
                                   // from trace A

    memset(curve, 0, 800 * sizeof(S16));
    memcpy(curve, GPIB_read_BIN(800 * sizeof(S16), TRUE, FALSE),
           800 * sizeof(S16));

    for (S32 i = 0; i < 800; i++) {
      curve[i] = ((curve[i] & 0xff00) >> 8) + ((curve[i] & 0x00ff) << 8);
      trace[i] = (DOUBLE(S16(curve[i])) / 100.0) +
                 state.A_offset; // HP 70K measurement units are dBm * 100
    }
    ...
    memcpy(curve, GPIB_read_BIN(481 * sizeof(S16), TRUE, FALSE),
           481 * sizeof(S16));
    ...
    SA_printf("TB0S2"); // Disable trace 'B' visibility from last sweep, and
    ...
    SA_printf("SABO"); // Store trace A to trace B, then initiate binary dump of
                       // trace B
    ...
    memcpy(curve, GPIB_read_BIN(1002 * sizeof(S16), TRUE, FALSE),
           1002 * sizeof(S16));

    DOUBLE offset = state.min_dBm;
    DOUBLE scale =
        (state.max_dBm - state.min_dBm) / 1000; // bottom=0, ref=1000, top=1023
    DOUBLE grat_offset = 0.0;

    for (S32 i = 0; i < 1001; i++) {
      curve[i + 1] =
          ((curve[i + 1] & 0xff00) >> 8) + ((curve[i + 1] & 0x0003) << 8);
      trace[i] = ((DOUBLE(curve[i + 1]) + grat_offset) * scale) + offset;
    }

    state.dBm_values = trace;
    state.n_trace_points = 1001;
  } else if ((state.type == SATYPE_HP358XA) || (state.type == SATYPE_E4406A) ||
             (state.type == SATYPE_FSE) || (state.type == SATYPE_FSU) ||
             (state.type == SATYPE_FSP)) {
    //
    // Abort any sweep in progress and trigger the next one, then wait
    // for it to complete and request a trace dump
    //
    // On supported models, turn off the front-panel display if high-speed
    // acquisition is requested
    //

    if ((state.type == SATYPE_FSE) || (state.type == SATYPE_FSU) ||
        (state.type == SATYPE_FSP)) {
    ...
    // is what we expect
    //
    // (If n_expected_trace_points is zero, we're making an initial call to see
    // how many points the E4406A VSA will actually return, so don't flag as an
    // error)
    //
    ...
    if ((state.type == SATYPE_FSE) || (state.type == SATYPE_FSU) ||
        (state.type == SATYPE_FSP) || (state.type == SATYPE_E4406A)) {
      static SINGLE curve[MAX_N_PTS];

      memset(curve, 0, n_expected_trace_points * sizeof(SINGLE));

      memcpy(
          curve,
          GPIB_read_BIN(n_expected_trace_points * sizeof(SINGLE), TRUE, FALSE),
      ...
      memset(curve, 0, n_expected_trace_points * sizeof(U32));

      memcpy(curve,
             GPIB_read_BIN(n_expected_trace_points * sizeof(U32), TRUE, FALSE),
             n_expected_trace_points * sizeof(U32));

      for (S32 i = 0; i < n_expected_trace_points; i++) {
        U32 swapped =
            (((curve[i] & 0x000000ff) << 24) | ((curve[i] & 0x0000ff00) << 8) |
             ((curve[i] & 0x00ff0000) >> 8) | ((curve[i] & 0xff000000) >> 24));
    ...
    memcpy(curve, GPIB_read_BIN(n_src_points * sizeof(S16), TRUE, FALSE),
           n_src_points * sizeof(S16));

    DOUBLE offset = state.min_dBm;
    DOUBLE scale = (state.max_dBm - state.min_dBm) /
                   12800; // bottom=1792, ref=14592, top=14592+
    ...
    memcpy(curve, GPIB_read_BIN(501 * sizeof(S16), TRUE, FALSE),
           501 * sizeof(S16));

    DOUBLE offset = state.min_dBm;
    DOUBLE scale = (state.max_dBm - state.min_dBm) /
                   2880; // bottom=512, ref=3392, top=3392+
    ...
    memcpy(curve, GPIB_read_BIN(701 * sizeof(S16), TRUE, FALSE),
           701 * sizeof(S16));
    ...
    memset(curve, 0, n_src_points * sizeof(S16));
    memcpy(curve, GPIB_read_BIN(n_src_points * sizeof(S16)),
           n_src_points * sizeof(S16));
    ...
    for (S32 i = 0; i < n_src_points; i++) {
      curve[i] = ((curve[i] & 0xff00) >> 8) + ((curve[i] & 0x00ff) << 8);
      trace[i] = (DOUBLE(S16(curve[i])) / 100.0) +
                 state.A_offset; // MS8604A measurement units are dBm * 100
    }
    ...
    for (S32 i = 0; i < 501; i++) {
      curve[i] = ((curve[i] & 0xff00) >> 8) + ((curve[i] & 0x00ff) << 8);
      trace[i] =
          (DOUBLE(S16(curve[i])) / 100.0) +
          state.A_offset; // MS256X/MS266X measurement units are dBm * 100
    }
    ...
    //
    // Acquire 32-bit LE IEEE754 float data from trace and swallow trailing
    // newline
    //
    ...
    S32 floats_per_sample = state.VSA_vect_mode ? 2 : 1;
    S32 bytes_per_sample = sizeof(SINGLE) * floats_per_sample;
    S32 n_record_bytes = state.VSA_n_record_samples * bytes_per_sample;

    //
    // Trigger a sweep and copy trace to register D1 when it's finished,
    // then fetch and verify the preamble
    //
    // (89400 is >10x faster in continuous sweep mode, but there seems no way to
    // copy new trace data to D1 unless we go into single-sweep mode? TODO)
    //

    SA_printf(":INIT:IMM;*WAI;:TRACE:COPY D1,TRACE1;:TRACE:DATA? D1");
    ...
    assert(atoi(size_text) == n_record_bytes);
    // if (atoi(size_text) != n_record_bytes)
    //    {
    //    n_record_bytes = atoi(size_text);      // should no longer be
    //    necessary with POINts? query in scalar mode
    //    }
    ...
    // newline
    //
    // All data arrives in BE order, and must be byte-swapped
    //

    static SINGLE curve[MAX_N_PTS];
    memset(curve, 0, n_record_bytes);

    memcpy(curve, GPIB_read_BIN(n_record_bytes, TRUE, FALSE), n_record_bytes);

    GPIB_read_ASC();

    for (S32 i = 0; i < state.VSA_n_record_samples * floats_per_sample; i++) {
      U32 swapped = (((((S32 *)curve)[i] & 0x000000ff) << 24) |
                     ((((S32 *)curve)[i] & 0x0000ff00) << 8) |
                     ((((S32 *)curve)[i] & 0x00ff0000) >> 8) |
                     ((((S32 *)curve)[i] & 0xff000000) >> 24));

      curve[i] = *(SINGLE *)&swapped;
    }

    //
    // Convert 89400-series trace data in peak volts to dBm
    //

    static DOUBLE trace[MAX_N_PTS];

    SINGLE *src = &curve[floats_per_sample * state.VSA_a0];

    if (state.VSA_vect_mode) {
      for (S32 i = state.VSA_a0; i <= state.VSA_a1; i++) {
        DOUBLE I = *src++;
        DOUBLE Q = *src++;

        DOUBLE Vpk = sqrt(I * I + Q * Q);
        DOUBLE Vrms = Vpk * 0.707107F;
        DOUBLE W = (Vrms * Vrms) / state.VSA_input_Z;
        DOUBLE dBW = 10.0F * log10(W);
    ...
    state.dBm_values = &trace[state.VSA_a0];
    state.n_trace_points = n_expected_trace_points;
    ...
    memset(curve, 0, n_expected_trace_points * sizeof(S32));

    memcpy(curve,
           GPIB_read_BIN(n_expected_trace_points * sizeof(S32), TRUE, FALSE),
...
bool WINAPI SA_configure_sweep(DOUBLE center_Hz, DOUBLE span_Hz,
                               DOUBLE ref_level_dBm, DOUBLE rbw_Hz) {
  if (state.type == SATYPE_HP8566A_8568A ||
    ...
    state.max_Hz = center_Hz + (span_Hz / 2.0);
    state.max_dBm = ref_level_dBm;
    state.min_dBm = state.max_dBm - (state.dB_division * state.amplitude_levels);
    state.CF_Hz = center_Hz;
    return TRUE;
  } else if (state.type == SATYPE_SCPI || state.type == SATYPE_FSP ||
             state.type == SATYPE_FSU || state.type == SATYPE_FSE ||
             state.type == SATYPE_N9900 || state.type == SATYPE_E4406A) {
    SA_query_printf(":SENS:FREQ:CENT %lf;:SENS:FREQ:SPAN %lf;*OPC?", center_Hz,
                    span_Hz);
    GPIB_read_ASC();
    SA_query_printf(":DISP:WIND:TRAC:Y:RLEV %lf;*OPC?", ref_level_dBm);
    GPIB_read_ASC();
    if (rbw_Hz <= 0.0)
      SA_query_printf(":SENS:BAND:RES:AUTO ON;*OPC?");
    else
      SA_query_printf(":SENS:BAND:RES %lf;*OPC?", rbw_Hz);
    GPIB_read_ASC();
    ...
    state.CF_Hz = center_Hz;
    return TRUE;
  } else if (state.type == SATYPE_TEK_490P || state.type == SATYPE_TEK_271X ||
             state.type == SATYPE_TEK_278X) {
    if (rbw_Hz <= 0.0)
      SA_printf("FREQ %lfHZ;SPAN %lfHZ;REFLVL %lfDBM;RESBW AUTO;", center_Hz,
                span_Hz, ref_level_dBm);
    else
      SA_printf("FREQ %lfHZ;SPAN %lfHZ;REFLVL %lfDBM;RESBW %lfHZ;", center_Hz,
                span_Hz, ref_level_dBm, rbw_Hz);
...
void WINAPI SA_resample_data(DOUBLE *src, S32 ns,
                             DOUBLE *dest, S32 nd, RESAMPLE_OP operation) {
  if (nd == ns) {
    //
