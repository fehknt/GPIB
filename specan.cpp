                    ...
                    bool      underscores_match_hyphens = FALSE)
{
   ...
   if (underscores_match_hyphens)
      {
      for (C8 *t = t1; *t; t++) if (*t == '-') *t = '_';
      for (C8 *t = t2; *t; t++) if (*t == '-') *t = '_';
      }
...
bool WINAPI GPIB_connect_to_analyzer(S32 address)
{
   ...
   if ((!_strnicmp(state.ID_string,"ID TEK/49",9))   ||
      ...
      if ((strstr(state.ID_string,"3588A") != NULL) ||
         ...
         {
         //
         // On HP 89400A series, SENS:SWEEP:POINTS? returns the number of points in the 
         // visible sweep, which is the alias-protected subset, e.g. 401 by default.  
         //
         // :SENS:FREQ:BAS? returns 0 if zoomed operation is in effect, but only in vector mode.  
         // Trace data is always complex if INST:SEL? returns VECT
         //
         ...
         switch (mode)
            {
            case 1: state.VSA_vect_mode = FALSE; break;     // SCALar
            case 3: state.VSA_vect_mode = TRUE;  break;     // VECTor

            default: 
               {
               alert("Error","VSA must be configured in standard scalar or vector mode");
               return FALSE;
               }
            }
         ...
         if (state.VSA_vect_mode)
            ...
            {
            //
            // In scalar mode, there seems to be no way to tell whether or not the measurement is 
            // "zoomed" except by looking at the number of points returned in the trace header.
            // TRACe:DATA:HEADer:POINts? D1 returns the number of points that were copied to the D1 
            // register, which will be the next larger value of (2^n)+1, or simply 2^n for zoomed data.  
            // See table on page 447 of GPIB manual.  Presumably we could use this method in both 
            // scalar and vector modes, more testing needed
            //
         ...
         //
         // Regardless of scalar/vector or zoomed/unzoomed mode, SENS:SWEEP:POINTS always reflects the 
         // number of valid, alias-protected samples that will ultimately be captured.  
         // TRACe:DATA:HEADer:POINts?  will always return the next higher power of 2, plus 1 if zoomed
         // 
         ...
         switch (state.n_trace_points)
            {                                                                         
            case 51:   if (state.VSA_zoomed) { state.VSA_n_record_samples = 64;   state.VSA_a0 = 7;   state.VSA_a1 = 57;   } else state.VSA_n_record_samples = 65;   break;
            case 101:  if (state.VSA_zoomed) { state.VSA_n_record_samples = 128;  state.VSA_a0 = 14;  state.VSA_a1 = 114;  } else state.VSA_n_record_samples = 129;  break; 
            case 201:  if (state.VSA_zoomed) { state.VSA_n_record_samples = 256;  state.VSA_a0 = 28;  state.VSA_a1 = 228;  } else state.VSA_n_record_samples = 257;  break; 
            case 401:  if (state.VSA_zoomed) { state.VSA_n_record_samples = 512;  state.VSA_a0 = 56;  state.VSA_a1 = 456;  } else state.VSA_n_record_samples = 513;  break; 
            case 801:  if (state.VSA_zoomed) { state.VSA_n_record_samples = 1024; state.VSA_a0 = 112; state.VSA_a1 = 912;  } else state.VSA_n_record_samples = 1025; break; 
            case 1601: if (state.VSA_zoomed) { state.VSA_n_record_samples = 2048; state.VSA_a0 = 224; state.VSA_a1 = 1824; } else state.VSA_n_record_samples = 2049; break; 
            case 3201: if (state.VSA_zoomed) { state.VSA_n_record_samples = 4096; state.VSA_a0 = 448; state.VSA_a1 = 3648; } else state.VSA_n_record_samples = 4097; break; 

            default:
               {
               alert("Error","Instrument must be configured for 51, 101, 201, 401, 801, 1601, or 3201 points");
               return FALSE;
               }
            }
         ...
         //
         // 89400 input load is set via INP:IMP, but in 1M mode, dBm can optionally be computed 
         // based on an arbitrary user-entered impedance
         //
      ...
      if (state.type == SATYPE_E4406A)
         ...
         //
         // NB: in I/Q spectrum mode, :SENS:FREQ:CENT? returns 1.000000000E+009, while the :SPAN? query is 
         // correct
         //
         ...
         if (state.type == SATYPE_FSP)
            GPIB_query(":SENS:SWE:POIN 501;*OPC?");      // FSP and FSU point counts are configurable, but we only support the default
         else if (state.type == SATYPE_FSU)
            GPIB_query(":SENS:SWE:POIN 625;*OPC?");      // (TODO: should set FSE to 500 as well?)
         ...
         if (((state.dB_division < 1) || (state.dB_division > 10))
               ||
             ((state.type == SATYPE_HP89400) && (strstr(GPIB_query("CALC:FORM?"), "MLOG") == NULL)))
            {
            SA_disconnect();
            state.type = SATYPE_INVALID;
      ...
      // 32-bit IEEE754 reals for HP 3588A/3589A, E4406A, 894xx, FSx, and the N9900 series handhelds
      //
      ...
      if (state.type == SATYPE_HP358XA)
         ...
         // by a :FETCH:SPEC4 query, so we must request a trace and see how many points it gives us
         //
...
void WINAPI SA_fetch_trace(void)
{
   ...
   if (state.type == SATYPE_SA44)
...
#if ASCII_ACQ
      SA_printf("O3;%s", state.trace_cmd);      // Request ASCII dBm values from specified trace (default = TA)
                               // (Slow)
      for (S32 i=0; i < 1001; i++)
         {
         do
      ...
      if (!state.hi_speed_acq)
         {
         SA_printf("O2;%s", state.trace_cmd);      // Request 16-bit binary display values from specified trace (default = TA)
                                                   // (Slightly slower but more precise than 8-bit binary mode)
         static U16 curve[1001];     
         ...
         {
         SA_printf("O4;%s", state.trace_cmd);      // Request 8-bit binary display values from specfied trace (default = TA)
                                                   // (Fast, but less precise)
         static U8 curve[1001];     
...
//      printf("Term = %d %d %d\n", foo[0], foo[1], foo[2]);
      ...
      //   {
      //   n_record_bytes = atoi(size_text);      // should no longer be necessary with POINts? query in scalar mode
      //   }
      ...
      memcpy(curve, 
             GPIB_read_BIN(n_record_bytes, TRUE, FALSE), 
             n_record_bytes);
