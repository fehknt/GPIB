...
struct STATS
{
   DOUBLE *samples;     // Optionally keep track of sample data (not needed for avg/variance/stdev but required for median)
   S32     capacity;
   S32     n;
   DOUBLE  maxval;
   DOUBLE  minval;
   DOUBLE  meanval;     //
   DOUBLE  Q;           // Welford method minimizes FP precision loss in variance calculations
   DOUBLE  sqsum;       //
...
void main(S32 argc, C8 **argv)
{
   ...
   if (argc < 3)
      ...
      printf("This program tests HP signal generators (8340A/B, 8656A/B, 8757A-D, 8642B/M,\n"
      ...
      printf("  sgentest 18 19 -8340 -fmax:20E9 -fmin:1E7 -amax:8 -amin:-20 -atol:4 -loss:4\n");
