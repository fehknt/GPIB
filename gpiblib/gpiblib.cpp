...
typedef ViStatus (__stdcall * P_viWrite)(ViSession vi, ViBuf buf, ViUInt32 cnt, ViPUInt32 retCnt);
typedef ViStatus (__stdcall * P_viRead)(ViSession vi, ViPBuf buf, ViUInt32 cnt, ViPUInt32 retCnt);
...
{
   if (hVISA != NULL) return TRUE;

   hVISA = LoadLibrary("visa32.dll");
   if (hVISA == NULL) return FALSE;

   p_viOpenDefaultRM = (P_viOpenDefaultRM) GetProcAddress(hVISA, "viOpenDefaultRM");
...
S32    WINAPI GPIB_online   (void)
{
   ...
   if (!INI_is_NI488 && !INI_is_VISA)
      ...
      GPIB_ID = ibfind(INI_setup_string);             // avoid errors due to partial uninstall of old NI drivers
      SetErrorMode(err);
...
{           
   ...
   if (!INI_is_NI488 && !INI_is_VISA)
      {
      if (prologix_listen_only_mode)   
         {                             // No queries, no buffer flushing, no address alteration... although we do 
         prologix_3012c = 0;           // send ++lon to set up listen-only mode, unless it's disabled in connect.ini
         prologix_V4 = 0;
         prologix_4040 = 0;
         prologix_6102 = 0;
         ...
         if (INI_enable_LON)
            {
            GPIB_write("++lon 1");     // Not supported for FW < 6.102, will need to disable in connect.ini if this causes problems
            }
         }
         ...
         if (device_address != -1)
            ...
            // occasionally if the Prologix adapter is already in controller mode with auto read off
            //
            // The ++auto commands don't seem to hurt anything in device mode, even though 
            // they aren't supported.  If we get "Unrecognized command", we'll go ahead and send ++mode 1
...
bool WINAPI GPIB_connect_ex(C8        *dev_address,
                            GPIBERR    handler, 
                            bool       clear, 
                            S32        timeout_msecs,
                            S32        board_address,
...
GPIB_CTYPE WINAPI GPIB_connection_type(void)
{
      ...
      if (lin_comment != NULL)
         ...
         *lin_comment = 0;
         ...
         while (1)
           ...
           if (!_stricmp(lexeme,"is_Prologix"))          INI_is_Prologix        = atoi(value) != 0;
      else if (!_stricmp(lexeme,"reset_to_local"))       INI_reset_to_local     = atoi(value) != 0;
      else if (!_stricmp(lexeme,"restore_auto_read"))    INI_restore_auto_read  = atoi(value) != 0;
      else if (!_stricmp(lexeme,"force_auto_read"))      INI_force_auto_read    = atoi(value) != 0;
      else if (!_stricmp(lexeme,"enable_LON"))           INI_enable_LON         = atoi(value) != 0;
   ...
   else if ((strstr(INI_setup_string, "::") != NULL) || (!_strnicmp(INI_setup_string, "USB", 3)))
      {
      INI_is_VISA = TRUE;
      INI_connection_type = GC_NI488; // Treat as NI488 for now to avoid breaking apps checking for serial/lan
      }
...
bool WINAPI GPIB_auto_read_mode (S32 set_mode)
{
   ...
   if (device_address != -1)
      {
      return TRUE;         // ++read not supported in device mode, where data always comes in automatically
      }
                             ...
                             bool   enable_board_configuration)
{
   ...
   if (INI_is_NI488 || INI_is_VISA)
      ...
      else if (enable_board_configuration && INI_is_NI488)   // (Added 2/07 for listen.exe: can turn this off if it breaks existing apps...)
         {                                  
                             ...
                             S32   *actual_len)
{
   ...
   if (!INI_is_NI488 && !INI_is_VISA)
      ...
      if (actual_len) *actual_len = retCnt;
      }
