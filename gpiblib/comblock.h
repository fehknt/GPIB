...
class SERBLOCK : public COMBLOCK
{
   ...
   void SetupCommState(char *config_text, bool Flow, C8 **error_text)
      ...
      if (!BuildCommDCB(config_text, &config))
         {
   ...
   void show_last_error(C8 **error_text, C8 *caption = NULL, ...)
      ...
      if (error_text)
         {
         sprintf(error_string, "%s -- %s", caption, (char *)lpMsgBuf);
...
class FTDBLOCK : public COMBLOCK
{
   ...
   void open_port(char **error_text)
      ...
      hPort = FT_W32_CreateFile(device,
                                GENERIC_READ | GENERIC_WRITE, 
                                0,
                                NULL,
                                OPEN_EXISTING, 
...
class TCPBLOCK : public COMBLOCK
{
   ...
   void open_port(char **error_text)
      ...
      struct addrinfo hints2, *result, *ptr;
      ...
      int status = getaddrinfo(server_address_name, port_str, &hints2, &result);

      if (status != 0)
         {
         if (inet_pton(AF_INET, server_address_name, &server_addr.sin_addr) != 1)
            {
            if (error_text != NULL)
               {
            ...
            show_last_error("Unable to resolve server name, error %d", status);
            close_port();
            exit(1);
            }
         }
         ...
         for (ptr = result; ptr != NULL; ptr = ptr->ai_next)
            ...
            if (ptr->ai_family == AF_INET)
               {
               memcpy(&server_addr.sin_addr, &((struct sockaddr_in*)ptr->ai_addr)->sin_addr, sizeof(struct in_addr));
               found = true;
               break;
               }
            }
         ...
         if (!found)
            ...
            if (error_text != NULL)
               {
               *error_text = "Unable to resolve server name (no IPv4 address)";
               close_port();
               return;
               }

            show_last_error("Unable to resolve server name (no IPv4 address)");
            close_port();
            exit(1);
            }
         }
...
// server_addr.sin_addr.S_un.S_un_b.s_b1++;     // (Connection error test)
      ...
      int conn_result = connect(server_connection,
...
//                 sizeof(recv_size));
      ...
      if (inet_ntop(AF_INET, &server_addr.sin_addr, connected_ip, sizeof(connected_ip)) == NULL)
         {
         strcpy(connected_ip, "UnknownIP");
         }
